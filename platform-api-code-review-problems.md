...
- **文件**: 
  ...
  - `management/infrastructure/InMemoryManagementConsoleService.java`（当前约 1290 行，local profile 部门和项目管理职责已拆出）
...
- **当前处理结果**:
  ...
  - `InMemoryManagementConsoleService` 已拆出 `InMemoryManagementDepartmentService` 和 `InMemoryManagementProjectService`，分别承接 local profile 部门列表/状态生命周期，以及项目列表、项目创建/更新、项目状态流转、项目成员添加/移除和对应审计。
