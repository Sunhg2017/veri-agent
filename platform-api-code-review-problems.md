...
| A1 | Service 单一职责拆分 | 进行中 | 已完成 WP3 `AssetService` 版本历史记录/查询职责拆分到 `AssetVersionHistoryService`；后续继续按 WP2/WP3/WP4 分模块拆分并保持接口兼容 |
...
1. 架构治理专项：A1，按模块逐步拆分并保持接口兼容；已推进 WP3 版本历史职责拆分，后续继续拆分资产导入导出、影响分析和模型接入调用编排等高耦合路径。
