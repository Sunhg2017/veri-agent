...
| A1 | Service 单一职责拆分 | 进行中 | 已完成 WP3 `AssetService` 版本历史记录/查询职责拆分到 `AssetVersionHistoryService`，影响分析图遍历、缺口计算和节点映射拆分到 `AssetImpactAnalysisService`，资产导入导出编排、CSV/JSON/OpenAPI 编解码和导入计划拆分到 `AssetImportExportService`；后续继续按 WP2/WP3/WP4 分模块拆分并保持接口兼容 |
...
1. 架构治理专项：A1，按模块逐步拆分并保持接口兼容；已推进 WP3 版本历史、影响分析和资产导入导出职责拆分，后续继续拆分原型同步和模型接入调用编排等高耦合路径。
