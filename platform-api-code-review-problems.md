...
| A2 | 贫血领域模型治理 | 进行中 | 已先将 WP3 需求/测试用例评审状态集合与转换规则迁入 `AssetReviewStatus` 领域枚举，并由领域记录暴露转换判断；后续继续迁移生命周期与版本规则 |
...
1. 架构治理专项：A1、A2，按模块逐步拆分并保持接口兼容；A2 已启动 WP3 评审状态规则内聚，后续继续处理生命周期与版本规则。
