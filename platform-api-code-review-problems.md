...
- **文件**: 
  ...
  - `modelaccess/application/ModelInvocationService.java`（当前约 493 行，provider 执行、预算/fallback 和调用记录落库已拆出）
  - `modelaccess/application/ModelProviderInvocationService.java`（当前约 534 行，承接 provider 执行、预算/fallback 和调用记录落库）
...
- **当前处理结果**:
  ...
  - `ModelAccessService` 已拆出 `ModelInvocationService`、`ModelProviderInvocationService`、`ModelCostAnalysisService`、`ModelProviderManagementService` 和 `PromptTemplateManagementService`，分别承接模型调用策略/路由、provider 执行/预算/fallback/调用记录、成本告警/日报聚合、供应商管理与 Prompt 版本/审批管理。
