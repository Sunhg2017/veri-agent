...
| A3 | 统一权限注解/AOP | 已完成 | 新增 `@RequirePermission` 注解和 AOP 切面；ModelAccessController、DocumentInputController 的普通权限入口改为注解校验；AssetController 保留资源级 `ResourceScope` 显式校验；DocumentInputActorResolver、DocumentCandidateWorkflowService、AssetVersionHistoryService 的主体解析已收敛到 `AuthorizationService`/actor resolver，application 层不再直接读取 `SecurityContextHolder` |
...
- **文件**: 
  ...
  - `modelaccess/application/ModelInvocationService.java`（当前约 494 行，provider 执行、预算/fallback 和调用记录落库已拆出）
  - `modelaccess/application/ModelProviderInvocationService.java`（当前约 376 行，承接 provider 尝试、fallback 和调用记录落库；预算窗口、预算超限和成本估算已拆出）
  - `modelaccess/application/ModelInvocationBudgetService.java`（当前约 205 行，承接 WP2 预算窗口、预算超限判断和 token 成本估算）
  - `asset/application/AssetService.java`（当前约 404 行，版本历史/版本回滚/生命周期/需求资产/API 资产/页面资产/业务流资产/测试用例资产/影响分析/导入导出/原型同步/追踪关系/测试用例步骤/项目审计上下文/响应 DTO 映射职责已拆出）
  - `documentinput/application/DocumentInputService.java`（当前约 923 行，导入编排/源管理/候选流程/发布编排/纠错反馈/响应 DTO 映射已拆出）
...
- **当前处理结果**:
  ...
  - `DocumentInputService` 已继续拆出 `DocumentImportService`，承接文本/二进制导入、导入记录查询、规则解析与模型解析合并、候选项落库、导入审计和指标记录。
  ...
  - `ModelAccessService` 已拆出 `ModelInvocationService`、`ModelProviderInvocationService`、`ModelInvocationBudgetService`、`ModelCostAnalysisService`、`ModelProviderManagementService` 和 `PromptTemplateManagementService`，分别承接模型调用策略/路由、provider 执行/fallback/调用记录、预算窗口/超限判断/成本估算、成本告警/日报聚合、供应商管理与 Prompt 版本/审批管理。
  ...
  - `AssetService` 已继续拆出 `AssetTestCaseService`，承接测试用例列表/读取、创建/更新、关联需求/API 项目校验、评审状态流转、版本历史入口、回滚入口和生命周期入口委托。
...
- **处理结果**: 已抽取 `@RequirePermission` + `RequirePermissionAspect`，普通控制器权限由注解统一触发；`AuthorizationService` 提供当前主体读取和服务令牌兼容逻辑；`AssetController` 的项目/资源级授权继续显式调用 `ResourceScope`，但底层主体读取已统一到 `AuthorizationService`。本次继续收敛 `DocumentCandidateWorkflowService` 和 `AssetVersionHistoryService` 的 actor 解析，application 层不再直接读取 `SecurityContextHolder`，仅过滤器与 `AuthorizationService` 保留安全上下文访问边界。
