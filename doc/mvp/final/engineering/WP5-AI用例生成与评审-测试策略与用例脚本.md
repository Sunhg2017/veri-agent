...
| WP5-FUNC-003C-1 | P0 | 查看上下文策略治理状态 | 健康接口无项目场景返回平台默认治理快照；项目/任务 effective snapshot 返回项目/环境 metadata 覆盖能力、审批要求和任务创建时固化状态。 |
| WP5-FUNC-003C-2 | P0 | 查看上下文策略运营 v2 状态 | 健康接口无项目场景返回平台默认-only 聚合快照；项目/任务 effective snapshot 返回 `PROJECT_ENVIRONMENT_OVERRIDE`、`PLATFORM_DEFAULT_PROJECT_ENVIRONMENT`、回退到平台默认、metadata 审批状态、项目/环境覆盖存储和审批流就绪状态。 |
| WP5-FUNC-003C-2A | P0 | 创建项目级上下文策略覆盖 | `ProjectOwner` 调用项目覆盖 API 创建 PENDING 记录；effective policy 在审批前仍使用平台默认；响应只返回 bounded 数字、状态和原因编码 captured 布尔值。 |
| WP5-FUNC-003C-2B | P0 | 审批项目级上下文策略覆盖 | `ProjectOwner` 审批后 effective policy 应用项目级覆盖，并返回 `PLATFORM_DEFAULT, PROJECT` 解析顺序；Developer 创建覆盖返回 403，其他项目负责人审批返回 403。 |
...
| WP5-FUNC-003F | P1 | 导出任务全量报告归档策略 | CSV 包含 `archivePolicy` 聚合行，只输出策略版本、保留天数、固定存储策略、审批要求、审批流/外发审批流/工单流转 ready、真实归档存储 ready、归档正文 stored、行级索引 ready、外发开关、策略跟踪、细节导出关闭和 aggregate-only，不输出归档内容、内部存储 key、归档路径、归档备注、审批说明或工单 URL。 |
...
| WP5-FUNC-003I | P1 | 导出任务全量报告 manifest | CSV 包含 `reportManifestPolicy` 和 `reportManifest` 聚合行，只输出策略版本、报告 schema 版本、字段集版本、manifest 模式、行数/完成状态跟踪、归档核验状态、manifest 前行数、aggregate-only 标记、明细行导出开关和完成状态；安全扫描通过后写入 `test_design_report_manifest`，保存 schema、字段集、manifest 模式、manifest 前后行数、完成状态和 CSV 内容 SHA-256 digest，不输出或保存候选 ID、trace、审计 ID 清单、行级完整性值、行级摘要或 CSV 正文。 |
| WP5-FUNC-003I-2 | P1 | 报告归档存储与审批流 | 安全扫描通过的任务报告导出后写入托管归档表、行级完整性索引表和初始 ARCHIVE 审批工单；`GET /report/archives`、`GET /report-archives/{id}/integrity` 和审批/备注接口只返回聚合元数据、状态、digest、行数、审批工单和备注时间线，不返回归档正文、内部存储 key、行 digest、候选 ID、trace ID 或审计 ID。 |
| WP5-FUNC-003I-1 | P1 | 重复导出任务全量报告 manifest | 同一任务重复导出时，每次安全扫描通过的导出都可按 `content_digest` 回查；若生成时间导致 CSV 内容不同，应保存多条互不重复的 manifest digest；相同 digest 的重放按唯一约束幂等。 |
...
| WP5-FUNC-003S | P0 | 维护细粒度队列告警订阅 | `GET/POST /api/v1/test-design/operations/queue-alert-subscriptions` 可按项目/prompt 读取和 upsert 订阅；响应只返回 bounded targetRef、阈值、启停状态和审计元数据，不返回 webhook URL、token、payload、任务 ID 或候选 ID。 |
| WP5-FUNC-003T | P0 | 人工重放 queued generation/publish 事件 | `POST /api/v1/test-design/operations/queued-events/replay` 仅允许项目 scope 内的有界重放；可分别触发 generation、publish 或 all，返回生成/发布任务事件和候选事件计数，不导出 event payload、队列消息体、任务 ID 或候选 ID。 |
| WP5-FUNC-003U | P0 | 查看发布补偿运行手册并手工运行补偿 | `GET /api/v1/test-design/operations/compensation-runbook` 返回补偿边界、候选范围、批大小和就绪状态；`POST /api/v1/test-design/operations/publish-compensation/run` 仅返回扫描/成功/失败/跳过计数，不导出候选 ID、资产 ID、sourceRef 或错误正文。 |
| WP5-FUNC-003V | P0 | 查看批量运营审计报表 | `GET /api/v1/test-design/operations/audit-report` 按项目/prompt 聚合队列告警变更、queued replay、补偿运行和 audit outbox requeue 计数；响应固定 aggregate-only，不返回审计行、traceId、模型调用 ID、outbox payload 或候选 ID。 |
| WP5-FUNC-003V-1 | P0 | 查看完整审计报表模板 | `GET /api/v1/test-design/operations/audit-report-template` 返回模板版本、字段集版本、运营汇总、告警订阅、模型观测聚合钻取、跨 WP 脱敏明细和导出红线字段；响应固定 aggregate-only，不返回运行时标识原值、payload 或 actor 标识。 |
| WP5-FUNC-003V-2 | P0 | 查看模型观测聚合钻取 | `GET /api/v1/test-design/operations/model-observation-drilldown` 按项目/prompt 返回 status/provider/model/routing group/prompt version/fallback 聚合桶，包含调用数、token、成本、延迟和 trace/job 存在性计数；不导出 traceId/jobId/invocationId 原值、provider 错误正文、actor service 或载荷预览。 |
| WP5-FUNC-003V-3 | P0 | 查看跨 WP 脱敏明细审计报表 | `GET /api/v1/test-design/operations/cross-wp-detail-audit-report` 按 WP 来源、分类和状态返回聚合行；不导出 WP1 audit row、WP2 invocation row、WP3 资产标识、任务 ID、候选 ID、sourceRef、payload 或 actor 标识。 |
...
| WP5-SEC-014 | P1 | 报告 manifest 携带候选 ID 清单、trace 清单、审计 ID 清单、行级完整性值或行级摘要 | 不允许进入任务报告导出；`reportManifestPolicy` 和 `reportManifest` 只允许导出策略版本、schema/字段集版本、行数跟踪、完成状态跟踪、归档核验、aggregate-only 和完成状态等固定聚合字段；`test_design_report_manifest` 只允许保存聚合行数、状态和 digest，不允许存在候选 ID、trace ID、审计 ID、行摘要、报告正文或 CSV 正文字段。 |
| WP5-SEC-014A | P1 | 报告 manifest 持久化绕过 aggregate-only 约束 | DB validation 必须验证 `aggregate_only=true`、`detail_rows_exported=false`、digest 为 64 位小写 hex，并检查 `test_design_report_manifest` 不存在 detail/id/raw report 禁止列。 |
| WP5-SEC-015 | P1 | 上下文策略治理或运营 v2 导出携带项目/环境覆盖规则、策略 diff、审批备注、工单 URL、原因编码原文或策略正文 | 不允许进入任务报告导出；治理和运营信息只允许导出固定版本、来源、状态、策略解析顺序、回退行为、开关、就绪布尔值和 aggregate-only 标记。 |
| WP5-SEC-015A | P0 | 上下文策略覆盖 API 响应泄露原因编码原文、策略正文、策略 diff、审批备注、工单 URL、覆盖规则或上下文正文 | 不允许进入响应；覆盖 API 只允许返回状态、bounded 数字、操作人摘要、时间戳和原因编码 captured 布尔值，DB 表不允许存在正文/diff/备注/URL/上下文字段。 |
| WP5-SEC-015B | P0 | 上下文策略覆盖表或 runtime DB role 绕过元数据边界 | DB validation 必须验证 `test_design_context_policy_override` 存在必要 bounded 元数据列、约束和索引，不存在策略正文、diff、审批备注、工单 URL、上下文正文、Prompt 或 request/response body 列；release role validation 必须要求 app role 可读写该表但不可 delete/truncate。 |
...
| WP5-FE-020 | P1 | 任务诊断归档策略摘要 | 展示 `archivePolicy` 的策略版本、保留天数、存储策略、审批要求、审批流/外发审批流/工单流转 ready、归档存储 ready、归档正文 stored、行级索引 ready、外发开关、保留策略跟踪和细节导出关闭；不展示归档正文、内部存储 key、归档路径、归档备注、审批说明或工单 URL。 |
...
if [[ "${WP5_GATE_MODE:-development}" == "release" || "${WP5_RELEASE_GATE:-0}" == "1" ]]; then
  if [[ "${WP5_RUN_HTTP_SMOKE:-0}" != "1" && "${WP5_RUN_HTTP_SMOKE:-0}" != "external" && "${WP5_RUN_HTTP_SMOKE:-0}" != "managed" && "${WP5_RUN_HTTP_SMOKE:-0}" != "auto" ]]; then
    echo "WP5 release gate requires HTTP smoke." >&2
    exit 2
  fi
  if [[ "${WP5_RUN_AI_EVAL:-0}" != "1" ]]; then
    echo "WP5 release gate requires AI quality evaluation." >&2
    exit 2
  fi
fi
...
# 9. 正式 publish。
# 10. 验证权限与资源作用域只读摘要为 aggregate-only。
# 11. 查询 WP3 test-cases 和 trace link 确认资产写入。
```
...
| 服务端 | 受限发布补偿后台只复用 sourceRef 回放和 trace link 修复，不进入首次创建或高相似冲突自动决策。 |
...
```bash
mvn -B -pl platform-api -Dtest=TestDesignControllerTest,TestDesignOpenApiContractTest,DbProfileRepositoryContractTest test
mvn -B -pl platform-api test
...
```bash
WP5_GATE_MODE=release WP5_RUN_HTTP_SMOKE=1 WP5_RUN_AI_EVAL=1 bash scripts/wp5_quality_gate.sh
WP5_GATE_MODE=release WP5_RUN_HTTP_SMOKE=external WP5_SMOKE_BASE_URL=http://127.0.0.1:8080 WP5_RUN_AI_EVAL=1 bash scripts/wp5_quality_gate.sh
```

非发布模式下可按影响面追加 AI 质量评测：
