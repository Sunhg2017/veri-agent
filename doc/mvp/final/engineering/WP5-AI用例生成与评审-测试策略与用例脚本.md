...
| 当前口径 | 已纳入 WP5 后端、前端、DB validation、Prompt 趋势、准出运营摘要、显式上下文装配、上下文裁剪策略配置、上下文策略治理状态快照、上下文策略运营就绪快照、任务诊断上下文策略摘要、任务报告生成编排策略聚合行、任务报告上下文聚合行、任务报告上下文装配策略聚合行、任务报告上下文策略治理聚合行、任务报告上下文策略运营就绪聚合行、任务报告模型观测策略聚合行、任务报告质量准出阈值策略聚合行、任务报告导出审计策略聚合行、任务报告安全扫描策略聚合行、任务报告归档策略聚合行、Prompt 校准策略聚合行、发布补偿策略聚合行、任务报告 manifest 聚合行和任务报告导出安全扫描的自动化验证入口 |
| 版本 | v2.4 |
| 日期 | 2026-05-30 |
...
| WP5-FUNC-003D-2 | P0 | 导出任务全量报告上下文策略运营就绪信息 | CSV 包含 `contextPolicyOperations` 聚合行，只输出平台默认运营模式、项目/环境覆盖存储就绪状态、审批流就绪状态、策略快照固化状态和禁止导出标记，不输出策略 diff、审批备注、工单 URL、项目/环境覆盖规则或策略正文。 |
...
| WP5-FUNC-003I | P1 | 导出任务全量报告 manifest | CSV 包含 `reportManifest` 聚合行，只输出报告 schema 版本、字段集版本、manifest 前行数、aggregate-only 标记、明细行导出开关和完成状态，不输出候选 ID、trace、审计 ID 清单或行级摘要。 |
...
| WP5-FUNC-003M | P1 | 导出任务全量报告模型观测策略 | CSV 包含 `modelObservationPolicy` 聚合行，只输出策略版本、聚合观测模式、WP2 调用引用跟踪、trace/job/routing/token/latency/cost/fallback 是否存在和路由/token/成本/延迟聚合计数，不输出模型调用 ID、异步 job ID、traceId 原值、载荷预览、原始 Prompt、provider 错误正文或 actor 服务。 |
...
| WP5-SEC-014 | P1 | 报告 manifest 携带候选 ID 清单、trace 清单、审计 ID 清单或行级摘要 | 不允许进入任务报告导出；manifest 只允许导出版本、行数、aggregate-only 和完成状态等固定聚合字段。 |
| WP5-SEC-015 | P1 | 上下文策略治理或运营就绪导出携带项目/环境覆盖规则、策略 diff、审批备注、工单 URL 或策略正文 | 不允许进入任务报告导出；治理和运营信息只允许导出固定版本、来源、状态、开关、就绪布尔值和 aggregate-only 标记。 |
...
| WP5-SEC-019 | P1 | 模型观测策略导出携带 invocationId、jobId、traceId 原值、请求/响应预览、原始 Prompt、provider 错误正文或 actor 服务 | 不允许进入任务报告导出；模型观测策略只允许导出固定版本、观测模式、布尔边界和路由/token/成本/延迟聚合计数。 |
