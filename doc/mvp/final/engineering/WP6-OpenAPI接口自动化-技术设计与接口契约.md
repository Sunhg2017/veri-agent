...
- Quality Gate M7/M8：`scripts/wp6_quality_gate.sh` 聚合脚本语法、OpenAPI fixture smoke、WP6 后端/OpenAPI 测试、前端 WP6 helper/权限测试、前端构建和 DB validation；支持 `WP6_QUALITY_GATE_PLAN_ONLY=1` 输出计划，开发模式默认不启 runner；显式 `WP6_RUNNER_SMOKE=managed|pytest|auto|external` 时调用 runner smoke。
...
本轮未实现：异步 cancel smoke 和复杂页面 Playwright smoke。这些仍按研发任务拆解的 M8/P1 继续推进；发布模式下 WP6 quality gate 会要求显式 runner smoke，当前可通过 service contract smoke、secretRef digest/SecretProvider/header 注入测试、Pytest subprocess adapter 契约测试、取消 API 幂等测试和基础 Managed HTTP loopback smoke 防止执行分支、脱敏、allowlist、取消入口和 HTTP/Pytest 探测回归。
