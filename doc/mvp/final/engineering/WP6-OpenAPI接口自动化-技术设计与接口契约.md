...
| `POST` | `/specs/{id}/archive` | `apiAutomation:import` | 归档规格源，保留脱敏解析证据但阻断后续重解析、diff、sync 和生成 |
...
| `api_automation.spec.archived` | OpenAPI 源归档 |
...
- 后端：新增 `/api/v1/api-automation/health`、`/specs` 创建/列表、`/specs/{id}` 详情、`/specs/{id}/parse` 重解析和 `/specs/{id}/archive` 归档；归档保留脱敏 spec 与 endpoint snapshot 证据，但后续 parse/diff/sync/sync-preview/generation 全部按状态机返回 `INVALID_STATE`。
...
- Runner M6/M8：新增 `ApiAutomationRunnerPort` validate/run/cancel 契约、默认 Disabled adapter、基础 Managed HTTP adapter 和显式 Pytest runner adapter，`POST /runs` 在脚本包 `APPROVED`、静态校验通过、baseUrl 安全策略通过后才进入执行路径；默认 disabled 时持久化 `BLOCKED/RUNNER_DISABLED` 摘要，显式 `runner-enabled=true` 默认装配 Managed HTTP adapter，显式 `runner-mode=pytest-subprocess` 时在临时目录重建最小 Pytest/httpx 文件树并执行本地子进程，显式 `runner-mode=pytest-docker-sandbox` 时复用同一脚本重建逻辑但通过 Docker CLI 在受限容器内执行；`secretRefs` 校验格式和数量后仅用完整引用调用 SecretProvider，审计和持久化只记录 `sha256:<digest>`，运行期只注入受控 `X-VA-WP6-Secret-N` header。
...
- Pytest Runner M8：当前 adapter 支持 `runner-mode=pytest-subprocess` 和 `runner-mode=pytest-docker-sandbox` 两条显式路径；命令默认为 `python3 -m pytest` 且不经 shell 执行。子进程模式在宿主机临时目录执行；Docker 沙箱模式通过 `runner-sandbox-command/image/network` 配置构造 `docker run --rm`，启用只读根文件系统、`tmpfs /tmp`、`tmpfs /run`、`--cap-drop ALL`、`--security-opt no-new-privileges`、`--pids-limit 256`、`--memory 256m`、`--cpus 1` 和 `--user 65534:65534`，把临时工作目录 bind mount 到 `/workspace`。两条路径都只写入本次 case 元数据重建的 Pytest/httpx 文件树，执行后删除；通过 `WP6_RUNNER_SECRET_HEADERS_JSON` 和 `WP6_RUNNER_SECRET_VALUE_N` 向 Pytest fixture 注入受控 header；结果从 JUnit XML 聚合为 case-level status/duration/errorCode，不持久化生成源码、stdout/stderr 原文、请求/响应正文或 secret。当前持久化 `runnerMode` 仍沿用既有 `EXTERNAL` 枚举，沙箱路径通过 health policy 和 assertion summary 的 `runnerAdapter/sandboxed` 字段标识。
