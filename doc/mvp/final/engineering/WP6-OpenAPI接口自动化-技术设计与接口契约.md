...
本轮未实现：异步 cancel smoke。该项仍按研发任务拆解的 M8/P1 继续推进；发布模式下 WP6 quality gate 会要求显式 runner smoke，当前可通过 service contract smoke、secretRef digest/SecretProvider/header 注入测试、Pytest subprocess adapter 契约测试、取消 API 幂等测试、基础 Managed HTTP loopback smoke 和 WP6 Playwright smoke 防止执行分支、脱敏、allowlist、取消入口、HTTP/Pytest 探测和前端主链路回归。
