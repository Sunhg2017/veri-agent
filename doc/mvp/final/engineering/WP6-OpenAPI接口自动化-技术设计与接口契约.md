...
- Runner M6：新增 `ApiAutomationRunnerPort` validate/run/cancel 契约、默认 Disabled adapter 和基础 Managed HTTP adapter，`POST /runs` 在脚本包 `APPROVED`、静态校验通过、baseUrl 安全策略通过后才进入执行路径；默认 disabled 时持久化 `BLOCKED/RUNNER_DISABLED` 摘要，显式 `runner-enabled=true` 时装配 Managed HTTP adapter；`secretRefs` 校验格式和数量后仅用完整引用调用 SecretProvider，审计和持久化只记录 `sha256:<digest>`，Managed HTTP runner 运行期只注入受控 `X-VA-WP6-Secret-N` header。
...
本轮未实现：Pytest 子进程型 runner、Pytest secretRef env/header 映射、异步 cancel smoke 和复杂页面 Playwright smoke。这些仍按研发任务拆解的 M8/P1 继续推进；发布模式下 WP6 quality gate 会要求显式 runner smoke，当前可通过 service contract smoke、secretRef digest/SecretProvider/header 注入测试、取消 API 幂等测试和基础 Managed HTTP loopback smoke 防止执行分支、脱敏、allowlist、取消入口和 HTTP 探测回归。
