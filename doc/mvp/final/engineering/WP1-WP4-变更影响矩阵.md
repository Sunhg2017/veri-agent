...
| WP1 SecretProvider：`secret_reference`、用途、作用域、过期、provider 状态、外部 Vault/KMS resolve/health/签名认证配置 | WP1 | WP4，后续 WP2 | `bash scripts/wp4_binary_document_smoke.sh`；`mvn -B -pl platform-api -Dtest=DocumentWebhookSecretResolverTest,ExternalSecretProviderTest,DocumentInputControllerTest test`；`bash db/validation/run_wp1_db_validation.sh` | 明文、secretRef、Bearer token、签名密钥不得进入响应、审计、日志或健康摘要；生产 fallback 关闭后必须拒绝未解析引用；外部 provider 不可用时要能通过健康摘要和指标定位 |
...
| WP2 provider 配置：`providerType`、`baseUrl`、`apiKeyRef`、priority、timeout、成本字段 | WP2 | WP2、WP4 AI 解析 | `mvn -B -pl platform-api -Dtest=ModelAccessControllerTest,OpenAiCompatibleModelProviderClientTest test`；`WP2_SERVICE_TOKEN=local-model-access-token bash scripts/wp2_model_access_smoke.sh` | 当前 external provider key 使用 `env:VARIABLE_NAME`；误填 secret 明文会被契约和 runbook 阻断 |
| WP2 invocation 契约：promptKey、messages、sensitivityLevel、allowPublicModel、响应 `providerName/modelName/totalCost` | WP2 | WP4 AI 解析、后续智能 WP | `mvn -B -pl platform-api -Dtest=ModelAccessControllerTest,DocumentInputModelParseControllerTest test`；`bash scripts/wp4_ai_parse_quality_eval.sh` | WP4 依赖 `modelInvocationId`、`modelProviderName`、`modelName` 做候选追踪 |
