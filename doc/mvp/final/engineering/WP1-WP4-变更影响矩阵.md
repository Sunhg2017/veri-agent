...
| WP1 SecretProvider：`secret_reference`、用途、作用域、过期、provider 状态、外部 Vault/KMS resolve/health 配置 | WP1 | WP4，后续 WP2 | `bash scripts/wp4_binary_document_smoke.sh`；`mvn -B -pl platform-api -Dtest=DocumentWebhookSecretResolverTest,ExternalSecretProviderTest,DocumentInputControllerTest test`；`bash db/validation/run_wp1_db_validation.sh` | 明文不得进入响应、审计、日志或健康摘要；生产 fallback 关闭后必须拒绝未解析引用；外部 provider 不可用时要能通过健康摘要和指标定位 |
