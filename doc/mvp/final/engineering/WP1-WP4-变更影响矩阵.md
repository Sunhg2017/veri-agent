...
| WP1 audit 写入契约：action、resourceType、result、traceId、before/after/diff | WP1 | WP2、WP3、WP4 | `mvn -B -pl platform-api test`；`bash scripts/wp1_db_profile_smoke.sh`；`bash scripts/wp_all_integration_test.sh` | audit 写失败不应污染主业务；失败要能通过日志或指标定位 |
...
| WP2 Prompt 审批契约：`highRisk`、`approvalStatus`、`approvedBy`、`approvedAt`、`approvalNote`、`/prompts/{id}/approve`、`/prompts/{id}/reject` | WP2 | WP2 管理台、WP4 AI 解析 Prompt 变更流程 | `mvn -B -pl platform-api -Dtest=ModelAccessControllerTest,ModelAccessOpenApiContractTest test`；`cd portal-web && npm test`；`bash scripts/wp2_model_quality_eval.sh` | 高风险 Prompt 未审批不能激活；审批字段不得包含 prompt 明文之外的敏感密钥；Prompt 变更后必须跑评测 |
