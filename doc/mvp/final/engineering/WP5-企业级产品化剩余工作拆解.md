...
| 当前状态 | WP5 已具备事件驱动规则模板生成、运行中超时回收、候选服务端分页评审、批量字段编辑、候选/发布摘要导出、dryRun、发布到 WP3 的最小闭环；仍未达到企业级产品完成态 |
| 版本 | v0.15 |
| 日期 | 2026-05-28 |
...
| P1 | 导出和报告摘要 | 产品经理、前端工程师 | 已补前端当前候选页、已选候选和发布结果 CSV 摘要导出，包含状态/覆盖/优先级/质量标记统计；仍缺服务端按完整筛选条件的全量导出、评审历史归档和导出审计。 | 导出不含原始 Prompt、密钥、token 或隐私字段。 |
...
| M4 前端企业化 | 用户在页面完成主流程 | 任务工作台、候选评审、发布预览、错误态、摘要导出 | 前端工程师 | 已具备任务/候选筛选、候选服务端分页联动、批量评审、批量字段编辑、批量/发布二次确认摘要、选择性发布、候选/发布摘要导出、候选质量提示、冲突摘要和 WP3 跳转；仍需运营化体验补齐。 |
...
| 导出泄露敏感信息 | 候选/发布摘要导出仅使用白名单字段，并对错误和评审摘要中的明显 secret/token/cookie/bearer 模式做本地脱敏；不导出描述、步骤正文、前置条件、原始 Prompt 或完整模型输入。 | 后续补服务端完整筛选导出时必须接入统一权限 scope、导出审计和敏感词扫描。 |
