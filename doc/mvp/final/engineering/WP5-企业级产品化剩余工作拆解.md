...
| 重试或重复事件导致候选重复 | 重试按 duplicateKey 跳过已有候选；异步生成事件消费通过 `QUEUED -> RUNNING` 条件认领，重复事件只返回当前任务；异步发布事件通过候选级 `PUBLISH_QUEUED -> PUBLISHING` 条件认领，重复事件不会重复创建 WP3 用例或追加发布记录；恢复扫描只重发仍排队的候选；跨 WP 统一运营看板已聚合展示 scope/audit/outbox 状态并提供受限 requeue。 | 后续补更细队列告警订阅和运行手册。 |
| 任务停留排队或运行态 | 任务先持久化为 `QUEUED`，提交事件后后台消费；恢复扫描会重发排队事件，并将超时 `RUNNING` 标记为可重试失败；正式发布先进入 `PUBLISH_QUEUED/PUBLISHING`，发布恢复扫描会重发排队候选、将超时 `PUBLISHING` 候选标记失败并收敛任务瞬态状态；HTTP smoke 和前端均按异步状态轮询。 | 后续补更完整队列运营告警和人工重放面板。 |
| 发布重复创建 WP3 用例 | 本次已按 `sourceRef=wp5:{candidateId}` 识别已存在用例并链接，新增 AI `sourceRef` 部分唯一索引、迁移前重复数据预检、服务层事务级锁、失败重试补链、受限补偿后台、同需求高相似用例检测、可配置相似度阈值、人工冲突链接入口、异步发布候选级条件认领和资产冲突运营台集中闭环。 | 后续继续补更完整跨 WP 明细审计报表和告警。 |
...
| 失败原因难定位 | 任务响应已从 WP2 调用日志和异步 job 读取脱敏模型观测摘要；前端任务诊断面板展示调用 ID、traceId、成本、延迟、token、fallback、错误码、脱敏错误摘要、`inputDigest`、`idempotencyKey` 和上下文键摘要；任务本域审计链摘要展示 WP5 任务、评审和发布记录的最近事件、失败冲突和说明覆盖，跨 WP 统一运营看板已串联 WP1 审计、WP2 调用、WP5 任务和 WP3 发布引用的聚合计数，便于人工定位回放或失败链路。 | 后续补更完整明细审计报表模板、告警订阅和模型观测聚合钻取。 |
...
| 发布失败产生部分成功 | 已记录逐候选 publish record；若 WP3 用例已创建但 trace link 缺失，人工重试和受限后台补偿都会按 sourceRef 找回用例并补建链接，候选保留失败原因和资产 ID；异步发布中断会由恢复扫描把超时 `PUBLISHING` 候选转为 `FAILED` 并允许人工重试；任务报告 `publishCompensationPolicy` 会聚合展示回放、补链、重试、后台补偿和人工链接能力边界，资产冲突运营台已集中处理人工冲突链接，不输出 sourceRef、资产 ID 或错误正文。 | 后续补更完整补偿告警、运行手册和批量运营审计报表。 |
...
| 审计链策略被误解为真实跨 WP 看板 | `auditChainPolicy` 只声明当前 WP5 本域聚合、WP1 审计写入、WP2 调用引用和 WP3 发布引用边界，固定输出 `crossWpAuditDashboardReady=true`、`auditOutboxReplayDashboardReady=true` 和 `aggregateOnly=true`；任务诊断与报告不导出审计事件明细、候选 ID 清单、平台审计标识原值、traceId 原值、模型调用 ID 原值、发布 sourceRef 或资产 ID 原值。 | 后续补更完整的跨 WP 审计报表、告警订阅和模型观测明细聚合。 |
