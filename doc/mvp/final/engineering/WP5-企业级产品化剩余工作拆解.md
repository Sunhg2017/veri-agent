...
| P0 | 输出 Schema 和质量门禁 | 质量工程师 | 已补候选 JSON Schema、模型原始输出 JSON Schema/parser、生成/编辑落库前质量门禁、重复键和敏感泄露阻断、任务报告 `readinessPolicy` 准出阈值运营聚合，`evaluationCorpusPolicy` 评测语料运营策略在 health、任务响应、`contextSummary`、模型 `contextPacking`、任务报告和前端诊断中的聚合快照，以及 `releaseReadinessPolicy` 发布准出审批策略聚合快照；`scripts/wp5_quality_gate.sh` 已在发布准出模式强制要求 `WP5_RUN_AI_EVAL=1` 运行 golden set 基线；仍缺真实 WP2 provider 联调、评测语料运营后台、长期校准后台和可阻断发布流的真实审批工作流。 | 不合格输出不得静默落库；golden set 阈值可回归且发布准出不可静默跳过；运营报表能看到当前评测语料、准出阈值、advisory-only 发布准出边界与风险项。 |
