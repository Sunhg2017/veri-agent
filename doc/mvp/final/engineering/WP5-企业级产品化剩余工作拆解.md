...
| P1 | 审计与观测 | 服务端架构师 | 生成、评审、发布已有部分审计；任务响应已补脱敏模型观测摘要，前端诊断面板可查看 prompt/model、`modelInvocationId`、异步 job traceId、成本、延迟、token、fallback、`inputDigest`、`idempotencyKey`、错误摘要、创建/更新时间、上下文键摘要、上下文策略摘要、策略治理状态、策略运营 v2 状态、发布准出审批策略边界和 `auditChainPolicy` 跨 WP 审计链策略边界；`auditChainPolicy` 已同步到 health、任务响应、`contextSummary`、模型 `contextPacking`、任务诊断和任务报告，声明当前只做 WP5 本域聚合与 WP1 审计写入/WP2 调用/WP3 发布引用追踪，不导出审计事件明细、候选 ID 清单、平台审计标识原值、traceId 原值、模型调用 ID 原值、发布 sourceRef 或资产 ID 原值；任务报告已补 `modelObservationPolicy` 聚合行声明模型观测只导出路由、成本、延迟和 token 聚合信号，不导出 trace/job/invocation ID 原值、载荷预览或 provider 错误正文；任务报告已补本域审计链摘要，聚合 WP5 任务、评审记录和发布记录的事件数、说明覆盖、失败/冲突和最近事件；任务报告已补 `auditPolicy` 聚合行声明导出审计写入策略；仍缺跨 WP1 audit_log 的端到端统一审计链看板和 audit outbox 重放看板。 | 任务详情能定位模型调用、错误原因、上下文策略、发布准出边界和审计链。 |
...
8. 重复发布、重复候选和同需求高相似用例不会静默污染资产库。
9. 前端能覆盖 loading、empty、error、403、409、partial success、traceId 和发布结果。
