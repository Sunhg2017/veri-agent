...
| P1 | 前端产品化 | 前端工程师 | 已补任务筛选、候选筛选、候选分页、候选批量评审、勾选候选发布、冲突摘要、发布结果跳转 WP3 资产追踪和候选编辑字段级质量提示；仍需批量字段编辑和服务端分页联动。 | 用户无需 curl 可完成主链路；移动/窄屏无重叠。 |
...
| M4 前端企业化 | 用户在页面完成主流程 | 任务工作台、候选评审、发布预览、错误态 | 前端工程师 | 已具备任务/候选筛选、候选分页、批量评审、选择性发布、候选质量提示、冲突摘要和 WP3 跳转；仍需批量字段编辑和服务端分页联动。 |
...
| 批量评审部分失败 | 前端批量评审展示逐条失败摘要，并保留失败项选择，成功项从选择中移除。 | 后续增加更细的冲突定位、批量字段编辑预览和二次确认摘要。 |
