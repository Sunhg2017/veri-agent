...
| 当前状态 | WP5 已具备事件驱动规则模板生成、运行中超时回收、候选服务端分页评审、任务全量质量摘要、任务质量准出阈值摘要、可配置相似度冲突策略、评审历史页摘要、后端任务全量报告导出、任务模型观测摘要、服务端筛选导出、批量字段编辑、候选/发布摘要导出、dryRun、发布到 WP3 的最小闭环；仍未达到企业级产品完成态 |
| 版本 | v0.25 |
| 日期 | 2026-05-29 |
...
| P0 | 重复和冲突治理 | 服务端架构师 | 已补 `LINK_EXISTING`、同需求高相似 `DUPLICATE_REVIEW_REQUIRED` 和可配置相似度阈值策略，仍缺人工冲突处理台。 | 发布前能识别同 sourceRef、同需求高相似和已发布候选。 |
...
| 发布重复创建 WP3 用例 | 本次已按 `sourceRef=wp5:{candidateId}` 识别已存在用例并链接，新增 AI `sourceRef` 部分唯一索引、迁移前重复数据预检、服务层事务级锁和失败重试补链，并补同需求高相似用例检测和可配置相似度阈值。 | 继续补人工冲突处理和异步补偿后台。 |
...
| 模型响应结构漂移 | 本次对模型原始 JSON 做字段白名单、必填、枚举、步骤数量和敏感文本校验，非法响应在落库前阻断；前端任务诊断已展示 prompt/version、`modelInvocationId`、模型观测摘要、错误摘要和上下文键摘要。 | 接入真实 WP2 后把 parser 挂到模型调用结果，并继续补端到端审计链看板。 |
