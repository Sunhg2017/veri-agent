...
| P0 | 发布幂等和补偿 | 服务端架构师 | 已补 AI 生成用例 `sourceRef` 服务层回放、事务级锁和数据库唯一约束；仍缺失败补偿和跨 WP 事务边界说明。 | 重复发布不重复建用例；失败有记录和可重试策略。 |
...
| 发布重复创建 WP3 用例 | 本次已按 `sourceRef=wp5:{candidateId}` 识别已存在用例并链接，新增 AI `sourceRef` 部分唯一索引、迁移前重复数据预检和服务层事务级锁，并补同需求高相似用例检测。 | 继续补可配置相似度策略、人工冲突处理、失败补偿和跨 WP 事务边界说明。 |
...
| 发布失败产生部分成功 | 已记录逐候选 publish record，并通过 sourceRef 唯一约束降低重试重复建用例风险。 | 增加补偿计划和跨 WP 发布事务边界说明。 |
