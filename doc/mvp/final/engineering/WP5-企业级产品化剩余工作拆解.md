...
| 任务报告摘要被误解为全量准出报告 | “导出报告”已切换为后端全任务聚合，输出任务元数据、模型观测聚合字段、上下文规模与策略聚合字段、导出治理聚合字段、导出审计策略聚合字段、安全扫描策略聚合字段、归档策略聚合字段、Prompt 校准策略聚合字段、报告 manifest 聚合字段、全量候选质量摘要、全量评审历史摘要和发布记录摘要；不输出候选正文、步骤正文、评审评论、请求/响应 preview、trace/job 明细、受控上下文正文、审计事件明细、候选/trace/审计 ID 清单、安全扫描命中详情、样本明细、归档路径或审批备注，并写导出审计。 | 后续补真实归档存储、审批流、审计报表模板、样本集维护后台和敏感词扫描运营记录。 |
...
| 导出泄露敏感信息 | 候选服务端筛选导出、评审历史导出、后端任务全量报告导出、已选候选和发布摘要导出均使用白名单字段；候选筛选导出限制 500 条并写审计，任务报告只输出聚合字段并在返回前执行安全扫描；导出审计策略仅输出固定动作、资源、项目作用域和布尔开关，安全扫描策略仅输出固定枚举和布尔开关，归档策略仅输出有界数字、布尔值和固定枚举，Prompt 校准策略仅输出版本、来源、状态、计数和布尔开关，manifest 仅输出版本、行数和完成状态，不导出审计事件明细、候选/trace/审计 ID 清单、安全扫描命中片段、样本行、评审评论、归档路径、审批说明、描述、步骤正文、前置条件、原始 Prompt 或完整模型输入。 | 后续将真实归档存储状态和敏感词扫描记录纳入统一导出治理。 |
