...
| 发布重复创建 WP3 用例 | 本次已按 `sourceRef=wp5:{candidateId}` 识别已存在用例并链接，新增 AI `sourceRef` 部分唯一索引、迁移前重复数据预检、服务层事务级锁、失败重试补链、受限补偿后台、手工发布补偿运行手册、同需求高相似用例检测、可配置相似度阈值、人工冲突链接入口、异步发布候选级条件认领和资产冲突运营台集中闭环。 | 后续继续补真实归档存储和模型观测明细聚合。 |
...
| 审计链策略被误解为真实跨 WP 看板 | `auditChainPolicy` 只声明当前 WP5 本域聚合、WP1 审计写入、WP2 调用引用和 WP3 发布引用边界，固定输出 `crossWpAuditDashboardReady=true`、`auditOutboxReplayDashboardReady=true` 和 `aggregateOnly=true`；跨 WP 运营面板已补队列告警订阅、人工重放、补偿运行手册、批量运营审计报表、审计报表模板、模型观测聚合钻取和跨 WP 脱敏明细审计报表，但任务诊断与报告仍不导出审计事件明细、候选 ID 清单、平台审计标识原值、traceId 原值、模型调用 ID 原值、发布 sourceRef 或资产 ID 原值。 | 后续补真实外部通知渠道和真实归档运营报表。 |
