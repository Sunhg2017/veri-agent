...
| managed HTTP smoke 和 AI 评测环境依赖 | `WP5_GATE_MODE=release WP5_RUN_HTTP_SMOKE=1 WP5_RUN_AI_EVAL=1` 会启动 Docker PostgreSQL 和 Maven `platform-api`，并运行 WP5 golden set 基线，日志保存在 `build/wp5-http-smoke/` 和对应测试输出中；日常开发不设置发布模式时仍可跳过 HTTP smoke 与 AI 评测。 | CI 或受限环境可改用 `WP5_GATE_MODE=release WP5_RUN_HTTP_SMOKE=external WP5_SMOKE_BASE_URL=... WP5_RUN_AI_EVAL=1` 指向已部署服务，失败时保留日志并阻断发布；发布模式未配置 HTTP smoke 或 AI 评测会在脚本入口失败。 |
