...
| M5 准出运营 | 可持续评估和发布 | golden set、质量阈值、发布准出记录、观测字段 | 产品经理、质量工程师 | quality gate、smoke、评测报告可复现；发布准出模式会强制执行 HTTP smoke；新增异步生成专项测试、事件恢复专项测试、生成编排策略报告专项测试、模型观测摘要专项测试、发布准出审批策略专项测试、任务报告导出专项测试、任务全量质量摘要/准出阈值专项测试和 `QUEUED` 状态 DB validation。 |
...
| managed HTTP smoke 环境依赖 | `WP5_GATE_MODE=release WP5_RUN_HTTP_SMOKE=1` 会启动 Docker PostgreSQL 和 Maven `platform-api`，日志保存在 `build/wp5-http-smoke/`；日常开发不设置发布模式时仍可跳过 HTTP smoke。 | CI 或受限环境可改用 `WP5_GATE_MODE=release WP5_RUN_HTTP_SMOKE=external WP5_SMOKE_BASE_URL=...` 指向已部署服务，失败时保留日志并阻断发布；发布模式未配置 HTTP smoke 会在脚本入口失败。 |
