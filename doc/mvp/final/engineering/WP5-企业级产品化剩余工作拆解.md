...
| P0 | 输出 Schema 和质量门禁 | 质量工程师 | 已补候选 JSON Schema、模型原始输出 JSON Schema/parser、生成/编辑落库前质量门禁、重复键和敏感泄露阻断、任务报告 `readinessPolicy` 准出阈值运营聚合，以及 `releaseReadinessPolicy` 发布准出审批策略在 health、任务响应、`contextSummary`、模型 `contextPacking`、任务报告和前端诊断中的聚合快照；仍缺真实 WP2 provider 联调和可阻断发布流的真实审批工作流。 | 不合格输出不得静默落库；golden set 阈值可回归；运营报表能看到当前准出阈值、advisory-only 发布准出边界与风险项。 |
...
| P0 | 任务编排与幂等 | 项目经理、服务端架构师 | 已补 retry/cancel 契约、状态保护、创建任务幂等键、前端稳定幂等提交键、requestDigest 冲突检测、DB 事务锁、`QUEUED -> RUNNING` 条件认领、事件恢复扫描、运行中超时失败回收和任务报告 `generationOrchestrationPolicy` 编排策略聚合；仍缺跨 WP 补偿后台、多实例压测、队列 lag 指标和超时告警。 | 重试不重复污染候选；取消可阻断排队/运行中任务；重复请求、重复事件和运行中卡死可识别，运营报表能看到当前编排能力边界和缺口。 |
| P0 | 权限和资源作用域加固 | 服务端架构师、质量工程师 | 已补批量候选操作和候选导出按候选/任务项目 scope 鉴权；新增 `scopePolicy` 聚合快照并同步到 health、任务响应、`contextSummary.scopePolicy`、模型 `contextPacking.scopePolicy`、前端任务诊断和任务报告，固定声明任务/候选/批量/发布/异步生成按项目 scope、HTTP smoke 和质量评测项目隔离，以及候选 ID/角色规则/服务令牌原值不导出；仍缺跨 WP 统一作用域看板和评测语料运营后台。 | 项目角色不能操作其他项目候选；服务令牌调用可审计，运营报表能看到当前 scope 边界和缺口。 |
...
| P1 | 前端产品化 | 前端工程师 | 已补任务筛选、候选筛选、候选服务端分页联动、候选批量评审、批量字段编辑、批量/发布二次确认摘要、勾选候选发布、候选/发布摘要导出、后端任务全量报告导出、任务全量质量摘要、任务质量准出阈值摘要、Prompt 版本趋势面板、评审历史页摘要、人工反馈回流摘要、冲突搜索与单条/批量复用、发布结果跳转 WP3 资产追踪、候选编辑字段级质量提示、任务/候选生成来源区分、发布准出诊断摘要和排队/运行中任务自动刷新；仍需运营化体验补齐。 | 用户无需 curl 可完成主链路；移动/窄屏无重叠。 |
| P1 | AI 质量运营 | 产品经理、质量工程师 | 已补后端任务全量质量摘要、Prompt 版本趋势聚合接口、可配置默认准出阈值摘要、Prompt 版本级准出摘要、Prompt 准出状态分布、前端看板、人工反馈回流聚合、任务报告 `readinessPolicy` 准出阈值快照和任务报告 `promptCalibrationPolicy` 聚合快照，覆盖任务级和版本级状态/覆盖/优先级分布、可评审/可发布、步骤/最终预期完整性、低置信度、错误、缺需求、缺标题、重复键碰撞、人工修正、驳回/忽略、调优样本说明覆盖计数以及样本维护/长期校准未就绪状态；仍缺真实样本集维护后台和真实 Prompt 变更后的长期趋势校准。 | 每次 Prompt 变更可比较覆盖率、重复率、泄露数、有效步骤率、版本级准出状态、准出状态分布和部署阈值口径。 |
| P1 | 审计与观测 | 服务端架构师 | 生成、评审、发布已有部分审计；任务响应已补脱敏模型观测摘要，前端诊断面板可查看 prompt/model、`modelInvocationId`、异步 job traceId、成本、延迟、token、fallback、`inputDigest`、`idempotencyKey`、错误摘要、创建/更新时间、上下文键摘要、上下文策略摘要、策略治理状态、策略运营 v2 状态和发布准出审批策略边界；任务报告已补 `modelObservationPolicy` 聚合行声明模型观测只导出路由、成本、延迟和 token 聚合信号，不导出 trace/job/invocation ID 原值、载荷预览或 provider 错误正文；任务报告已补本域审计链摘要，聚合 WP5 任务、评审记录和发布记录的事件数、说明覆盖、失败/冲突和最近事件；任务报告已补 `auditPolicy` 聚合行声明导出审计写入策略；仍缺跨 WP1 audit_log 的端到端统一审计链看板。 | 任务详情能定位模型调用、错误原因、上下文策略、发布准出边界和审计链。 |
| P1 | 导出和报告摘要 | 产品经理、前端工程师 | 已补服务端按筛选条件的候选 CSV 导出、导出审计、前端筛选导出、已选候选、发布结果摘要导出、评审历史页摘要、后端任务全量报告导出、生成编排策略聚合行、模型观测聚合行、`modelObservationPolicy` 聚合行、上下文策略聚合行、`contextAssemblyPolicy` v2 聚合行、`contextPolicyOperations` 聚合行、`scopePolicy` 聚合行、`releaseReadinessPolicy` 聚合行、`readinessPolicy` 聚合行、`exportGovernance` 聚合行、`auditPolicy` 聚合行、`safetyScanPolicy` 聚合行、`archivePolicy` 聚合行、`promptCalibrationPolicy` 聚合行、`publishCompensationPolicy` 聚合行、`reportManifest` 聚合行和报告最终安全扫描；仍缺真实归档存储、归档审批流和更完整的审计报表。 | 导出不含事件 ID、事件 payload、队列消息体、恢复明细、幂等键、超时错误正文、原始 Prompt、密钥、token、受控上下文正文、模型调用 ID 原值、job/trace 原值、模型载荷预览、provider 错误正文、审计事件明细、候选/trace/审计 ID 清单、安全扫描命中详情、样本行、评审评论、归档路径、审批备注、策略覆盖规则、策略变更说明、工单 URL、候选级准出证据、阈值规则明细、发布候选 ID、资产用例 ID、sourceRef、trace 明细或发布错误正文，命中敏感模式时失败关闭。 |
...
| 任务报告发布补偿策略聚合 | 后端任务全量报告新增 `publishCompensationPolicy` 聚合行，固定输出补偿策略版本、回放键族、source-key 回放、部分 trace link 修复、失败候选重试、人工冲突链接支持状态、异步补偿后台和跨 WP 编排未就绪状态，以及 retry/link/manual/conflict/failed 聚合计数；不导出候选 ID、资产用例 ID、sourceRef、trace 明细、错误正文或评审说明，并将专项测试纳入 WP5 quality gate。 |
...
| 上下文策略运营 v2 聚合快照 | `contextPolicyOperations` 已升级为 v2，并在 health、任务响应、`contextSummary.policyOperations`、模型 `contextPacking.policyOperations`、前端任务诊断和后端任务全量报告中保持同一聚合快照；固定输出策略版本、平台默认运营模式、策略解析顺序、部署配置回退行为、审批状态、项目/环境覆盖存储未就绪、审批流未就绪、任务创建时策略快照已固化、策略 diff/审批备注/工单 URL/覆盖规则不导出和 aggregate-only 标记；不导出策略正文、项目/环境覆盖规则、审批备注或工单 URL，并将专项测试纳入 WP5 quality gate。 |
...
| 任务报告模型观测策略聚合 | 后端任务全量报告新增 `modelObservationPolicy` 聚合行，固定输出策略版本、`ROUTING_COST_LATENCY_AGGREGATE` 观测模式、WP2 调用引用跟踪、trace/job/routing/token/latency/cost/fallback 是否存在、禁止持久化 Prompt 载荷、禁止导出载荷预览、trace/job/invocation ID 原值和 provider 错误正文，以及路由元数据、token、成本、延迟聚合计数；不导出模型调用 ID、异步 job ID、traceId 原值、请求/响应预览、原始 Prompt、provider 错误正文或 actor 服务，并将专项测试纳入 WP5 quality gate。 |
...
| 任务报告 manifest 聚合 | 后端任务全量报告新增 `reportManifest` 聚合行，固定输出报告 schema 版本、字段集版本、manifest 追加前行数、aggregate-only 标记、明细行导出开关和完成状态；不导出候选 ID 清单、trace 清单、审计 ID 清单或行级摘要，并将专项测试纳入 WP5 quality gate。 |
| 任务报告生成编排策略聚合 | 后端任务全量报告新增 `generationOrchestrationPolicy` 聚合行，固定输出策略版本、同步/异步编排模式、条件认领、幂等创建回放、重复事件安全、事件恢复、运行中超时回收、显式重试、人工任务重试、运营缺口、恢复批次上限和状态/超时信号计数；不导出事件 ID、事件 payload、队列消息体、恢复明细、幂等键或超时错误正文，并将专项测试纳入 WP5 quality gate。 |
...
| 上下文策略治理状态快照 | 新增 `contextPolicyGovernance` 平台默认治理快照，health、任务响应、`contextSummary.policyGovernance`、模型 `contextPacking.policyGovernance`、前端任务诊断和任务报告 `contextPolicyGovernance` 聚合行保持一致；明确项目/环境覆盖关闭、审批流未就绪且只输出固定枚举/布尔状态，不导出审批备注、工单 URL 或策略正文。 |
| 上下文策略运营 v2 聚合快照 | 新增共享 `contextPolicyOperations` v2 聚合快照，把策略解析顺序、回退行为、审批状态、项目/环境覆盖存储、审批流、策略 diff 预览、审批备注、工单 URL 和覆盖规则导出状态拆成固定运营标记，并同步到 health、任务响应、`contextSummary`、模型载荷、前端诊断和任务报告；当前仅声明平台默认策略快照已在任务创建时固化，不引入真实运营后台或审批接口。 |
...
5. `releaseReadinessPolicy` 能在 health、任务诊断、模型载荷和任务报告中一致说明 advisory-only、发布阻断关闭、审批流未就绪、禁止自动发布和候选确认要求。
6. 重复发布、重复候选和同需求高相似用例不会静默污染资产库。
7. 前端能覆盖 loading、empty、error、403、409、partial success、traceId 和发布结果。
8. `mvn -B -pl platform-api test`、`cd portal-web && npm test`、`cd portal-web && npm run build` 与 WP5 专项 gate 按影响面执行并记录。
...
| 任务停留排队或运行态 | 任务先持久化为 `QUEUED`，提交事件后后台消费；恢复扫描会重发排队事件，并将超时 `RUNNING` 标记为可重试失败；任务报告 `generationOrchestrationPolicy` 输出编排能力、运行缺口和排队/运行/超时聚合信号；HTTP smoke 和前端均按异步状态轮询。 | 后续补队列 lag 指标、超时告警、人工重发入口和多实例压测证据。 |
...
| 任务报告摘要被误解为全量准出报告 | “导出报告”已切换为后端全任务聚合，输出任务元数据、生成编排策略聚合字段、`scopePolicy` 作用域边界字段、`releaseReadinessPolicy` 发布准出审批边界字段、模型观测聚合字段、`modelObservationPolicy` 观测边界字段、上下文规模、上下文装配策略与策略治理聚合字段、导出治理聚合字段、导出审计策略聚合字段、安全扫描策略聚合字段、归档策略聚合字段、Prompt 校准策略聚合字段、发布补偿策略聚合字段、报告 manifest 聚合字段、全量候选质量摘要、全量评审历史摘要和发布记录摘要；不输出事件 ID、事件 payload、队列消息体、恢复明细、幂等键、超时错误正文、候选正文、步骤正文、评审评论、请求/响应 preview、模型调用 ID 原值、trace/job 原值、provider 错误正文、受控上下文正文、审计事件明细、候选/trace/审计 ID 清单、安全扫描命中详情、样本明细、归档路径、审批备注、候选级准出证据、阈值规则明细、sourceRef、资产用例 ID、显式资产 ID、digest 值、schema、页面树、流程 JSON、候选 ID 列表、角色规则明细、服务令牌原值或发布错误正文，并写导出审计。 | 后续补真实归档存储、审批流、审计报表模板、样本集维护后台、补偿后台、跨 WP scope 看板和敏感词扫描运营记录。 |
| 模型观测摘要误泄露原文 | `modelObservation` 只返回调用状态、traceId/jobId、成本、延迟、token、路由和脱敏错误摘要；服务端测试验证不返回 WP2 request/response preview；任务报告已用 `modelObservationPolicy` 固定声明只导出 trace/job 是否存在、路由/token/成本/延迟计数和导出禁止项，不导出具体 traceId、jobId、invocationId、载荷预览、provider 错误正文或 actor 服务。 | 若未来增加更多 WP2 字段，必须继续走白名单 DTO、报表策略行和敏感模式回归测试。 |
...
| 发布失败产生部分成功 | 已记录逐候选 publish record；若 WP3 用例已创建但 trace link 缺失，重试会按 sourceRef 找回用例并补建链接，候选保留失败原因和资产 ID；任务报告 `publishCompensationPolicy` 会聚合展示回放、补链、重试和人工链接能力边界，但不输出 sourceRef、资产 ID 或错误正文。 | 增加补偿后台、重试调度和跨 WP 发布事务边界运行手册。 |
...
| 导出泄露敏感信息 | 候选服务端筛选导出、评审历史导出、后端任务全量报告导出、已选候选和发布摘要导出均使用白名单字段；候选筛选导出限制 500 条并写审计，任务报告只输出聚合字段并在返回前执行安全扫描；生成编排策略仅输出固定版本、模式、布尔开关和计数，作用域策略仅输出固定 scope 模型、项目隔离标记和禁止导出标记，发布准出审批策略仅输出固定决策模式、阈值来源、布尔边界和聚合计数，上下文装配策略仅输出固定版本、模式、布尔开关和计数，上下文策略运营 v2 仅输出策略版本、解析顺序、回退行为、审批状态、就绪布尔值和禁止导出标记，导出审计策略仅输出固定动作、资源、项目作用域和布尔开关，安全扫描策略仅输出固定枚举和布尔开关，归档策略仅输出有界数字、布尔值和固定枚举，Prompt 校准策略仅输出版本、来源、状态、计数和布尔开关，发布补偿策略仅输出版本、能力开关和计数，manifest 仅输出版本、行数和完成状态，不导出事件 ID、事件 payload、队列消息体、恢复明细、幂等键、超时错误正文、审计事件明细、候选/trace/审计 ID 清单、安全扫描命中片段、样本行、评审评论、归档路径、审批说明、策略覆盖规则、策略 diff、策略正文、候选级准出证据、阈值规则明细、发布 sourceRef、资产用例 ID、显式资产 ID、digest 值、schema、页面树、流程 JSON、候选 ID 列表、角色规则明细、服务令牌原值、发布错误正文、描述、步骤正文、前置条件、原始 Prompt 或完整模型输入。 | 后续将真实归档存储状态、补偿后台状态、跨 WP scope 看板和敏感词扫描记录纳入统一导出治理。 |
