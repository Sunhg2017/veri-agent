...
7. 重复发布、重复候选和同需求高相似用例不会静默污染资产库。
8. 前端能覆盖 loading、empty、error、403、409、partial success、traceId 和发布结果。
9. `mvn -B -pl platform-api test`、`cd portal-web && npm test`、`cd portal-web && npm run build` 与 WP5 专项 gate 按影响面执行并记录。
...
| 导出泄露敏感信息 | 候选服务端筛选导出、评审历史导出、后端任务全量报告导出、已选候选和发布摘要导出均使用白名单字段；候选筛选导出限制 500 条并写审计，任务报告只输出聚合字段并在返回前执行安全扫描；生成编排策略仅输出固定版本、模式、布尔开关和计数，作用域策略仅输出固定 scope 模型、项目隔离标记和禁止导出标记，评测语料策略仅输出固定版本、模式、阈值来源、能力开关和运营缺口，发布准出审批策略仅输出固定决策模式、阈值来源、布尔边界和聚合计数，上下文装配策略仅输出固定版本、模式、布尔开关和计数，上下文策略运营 v2 仅输出策略版本、解析顺序、回退行为、审批状态、就绪布尔值和禁止导出标记，导出审计策略仅输出固定动作、资源、项目作用域和布尔开关，安全扫描策略仅输出固定枚举和布尔开关，归档策略仅输出有界数字、布尔值和固定枚举，Prompt 校准策略仅输出版本、来源、状态、计数和布尔开关，发布补偿策略仅输出版本、能力开关和计数，manifest 仅输出版本、行数和完成状态，不导出事件 ID、事件 payload、队列消息体、恢复明细、幂等键、超时错误正文、审计事件明细、候选/trace/审计 ID 清单、安全扫描命中片段、评测语料行、样本行、评审评论、归档路径、审批说明、策略覆盖规则、策略 diff、策略正文、候选级准出证据、阈值规则明细、发布 sourceRef、资产用例 ID、显式资产 ID、digest 值、schema、页面树、流程 JSON、候选 ID 列表、角色规则明细、服务令牌原值、发布错误正文、描述、步骤正文、前置条件、原始 Prompt 或完整模型输入。 | 后续将真实归档存储状态、补偿后台状态、跨 WP scope 看板和敏感词扫描记录纳入统一导出治理。 |
