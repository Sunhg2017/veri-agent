...
| 当前状态 | WP5 已具备事件驱动规则模板生成、运行中超时回收、候选服务端分页评审、当前候选页质量摘要、评审历史页摘要、任务报告摘要导出、服务端筛选导出、批量字段编辑、候选/发布摘要导出、dryRun、发布到 WP3 的最小闭环；仍未达到企业级产品完成态 |
| 版本 | v0.20 |
| 日期 | 2026-05-28 |
...
| P1 | 导出和报告摘要 | 产品经理、前端工程师 | 已补服务端按筛选条件的候选 CSV 导出、导出审计、前端筛选导出、已选候选、发布结果摘要导出、评审历史页摘要和任务报告摘要导出；仍缺后端任务全量报告化导出、归档策略和更完整的审计报表。 | 导出不含原始 Prompt、密钥、token 或隐私字段。 |
...
| M4 前端企业化 | 用户在页面完成主流程 | 任务工作台、候选评审、发布预览、错误态、摘要导出 | 前端工程师 | 已具备任务/候选筛选、候选服务端分页联动、服务端筛选导出、批量评审、批量字段编辑、批量/发布二次确认摘要、选择性发布、候选/发布摘要导出、任务报告摘要导出、当前候选页质量摘要、评审历史页摘要、候选质量提示、冲突摘要和 WP3 跳转；仍需运营化体验补齐。 |
...
| 任务报告摘要被误解为全量准出报告 | 当前“导出报告”明确输出前端已加载范围：任务元数据、当前候选页质量摘要、当前评审页审计摘要和最近发布结果；不输出候选正文、步骤正文或评审评论。 | 后续由后端提供任务全量质量/审计/发布归档报告，统一权限 scope、导出审计、归档策略和敏感词扫描。 |
