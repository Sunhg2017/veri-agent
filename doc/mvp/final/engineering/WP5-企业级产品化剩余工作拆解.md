...
| 发布重复创建 WP3 用例 | 本次已按 `sourceRef=wp5:{candidateId}` 识别已存在用例并链接，新增 AI `sourceRef` 部分唯一索引、迁移前重复数据预检、服务层事务级锁、失败重试补链、受限补偿后台、同需求高相似用例检测、可配置相似度阈值、人工冲突链接入口和异步发布候选级条件认领。 | 继续补完整前端冲突运营台。 |
