...
| P1 | 审计与观测 | 服务端架构师 | 生成、评审、发布已有部分审计；任务响应已补脱敏模型观测摘要，前端诊断面板可查看 prompt/model、`modelInvocationId`、异步 job traceId、成本、延迟、token、fallback、`inputDigest`、`idempotencyKey`、错误摘要、创建/更新时间、上下文键摘要和上下文策略摘要；任务报告已补本域审计链摘要，聚合 WP5 任务、评审记录和发布记录的事件数、说明覆盖、失败/冲突和最近事件；任务报告已补 `auditPolicy` 聚合行声明导出审计写入策略；仍缺跨 WP1 audit_log 的端到端统一审计链看板。 | 任务详情能定位模型调用、错误原因、上下文策略和审计链。 |
| P1 | 导出和报告摘要 | 产品经理、前端工程师 | 已补服务端按筛选条件的候选 CSV 导出、导出审计、前端筛选导出、已选候选、发布结果摘要导出、评审历史页摘要、后端任务全量报告导出、模型观测聚合行、上下文策略聚合行、`contextAssemblyPolicy` 聚合行、`contextPolicyOperations` 聚合行、`readinessPolicy` 聚合行、`exportGovernance` 聚合行、`auditPolicy` 聚合行、`safetyScanPolicy` 聚合行、`archivePolicy` 聚合行、`promptCalibrationPolicy` 聚合行、`publishCompensationPolicy` 聚合行、`reportManifest` 聚合行和报告最终安全扫描；仍缺真实归档存储、归档审批流和更完整的审计报表。 | 导出不含原始 Prompt、密钥、token、受控上下文正文、审计事件明细、候选/trace/审计 ID 清单、安全扫描命中详情、样本行、评审评论、归档路径、审批备注、策略覆盖规则、策略变更说明、工单 URL、候选级准出证据、发布候选 ID、资产用例 ID、sourceRef、trace 明细或发布错误正文，命中敏感模式时失败关闭。 |
...
| 任务报告摘要被误解为全量准出报告 | “导出报告”已切换为后端全任务聚合，输出任务元数据、模型观测聚合字段、上下文规模、上下文装配策略与策略治理聚合字段、导出治理聚合字段、导出审计策略聚合字段、安全扫描策略聚合字段、归档策略聚合字段、Prompt 校准策略聚合字段、发布补偿策略聚合字段、报告 manifest 聚合字段、全量候选质量摘要、全量评审历史摘要和发布记录摘要；不输出候选正文、步骤正文、评审评论、请求/响应 preview、trace/job 明细、受控上下文正文、审计事件明细、候选/trace/审计 ID 清单、安全扫描命中详情、样本明细、归档路径、审批备注、sourceRef、资产用例 ID、显式资产 ID、digest 值、schema、页面树、流程 JSON 或发布错误正文，并写导出审计。 | 后续补真实归档存储、审批流、审计报表模板、样本集维护后台、补偿后台和敏感词扫描运营记录。 |
