...
| P1 | 前端产品化 | 前端工程师 | 已补任务筛选、候选筛选、勾选候选发布、冲突摘要、发布结果跳转 WP3 资产追踪和候选编辑字段级质量提示；仍需批量编辑和更完整分页。 | 用户无需 curl 可完成主链路；移动/窄屏无重叠。 |
...
| M4 前端企业化 | 用户在页面完成主流程 | 任务工作台、候选评审、发布预览、错误态 | 前端工程师 | 已具备任务/候选筛选、选择性发布、候选质量提示、冲突摘要和 WP3 跳转；仍需批量编辑和分页效率优化。 |
...
| 前端发布范围误选 | 发布面板默认覆盖全部可发布候选，勾选候选后仅发布勾选范围，并显示当前发布范围计数。 | 后续增加批量选择持久化、分页跨页选择提示和二次确认摘要。 |
