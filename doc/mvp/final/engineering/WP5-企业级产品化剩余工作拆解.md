...
| 跨 WP 运营增强 | `GET /api/v1/test-design/operations/cross-wp-dashboard` 已扩展 `queueAlerts`、`compensationRunbook` 和 `operationsAuditReport`；新增队列告警订阅 CRUD、人工 queued generation/publish 有界重放、发布补偿运行手册、手工补偿运行和批量运营审计报表接口，前端跨 WP 统一运营面板可按项目/prompt 维护告警目标、触发重放和补偿、查看聚合审计计数。响应固定 aggregate-only，不导出任务 ID、候选 ID、outbox payload、traceId、模型调用 ID、sourceRef、WP3 资产 ID、provider 错误或审批备注正文。 |
...
| 失败原因难定位 | 任务响应已从 WP2 调用日志和异步 job 读取脱敏模型观测摘要；前端任务诊断面板展示调用 ID、traceId、成本、延迟、token、fallback、错误码、脱敏错误摘要、`inputDigest`、`idempotencyKey` 和上下文键摘要；任务本域审计链摘要展示 WP5 任务、评审和发布记录的最近事件、失败冲突和说明覆盖，跨 WP 统一运营看板已串联 WP1 审计、WP2 调用、WP5 任务和 WP3 发布引用的聚合计数，并提供队列告警、人工重放、补偿运行手册和批量运营审计报表，便于人工定位回放或失败链路。 | 后续补模型观测聚合钻取和真实通知渠道。 |
...
| 审计链策略被误解为真实跨 WP 看板 | `auditChainPolicy` 只声明当前 WP5 本域聚合、WP1 审计写入、WP2 调用引用和 WP3 发布引用边界，固定输出 `crossWpAuditDashboardReady=true`、`auditOutboxReplayDashboardReady=true` 和 `aggregateOnly=true`；跨 WP 运营面板已补队列告警订阅、人工重放、补偿运行手册和批量运营审计报表，但任务诊断与报告仍不导出审计事件明细、候选 ID 清单、平台审计标识原值、traceId 原值、模型调用 ID 原值、发布 sourceRef 或资产 ID 原值。 | 后续补模型观测明细聚合和真实通知渠道。 |
