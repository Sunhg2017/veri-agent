...
| P0 | 企业级上下文装配 | 服务端架构师 | 已补 WP3 需求、WP4 来源摘要、追踪关联资产、历史用例摘要、显式 API/页面/业务流输入、`veri-agent.test-design.context-*` 驱动的裁剪策略快照，以及 health、任务响应、模型上下文打包、任务诊断、任务报告平台默认治理状态快照、`contextPolicyOperations` v2 运营聚合快照和 `contextAssemblyPolicy` 装配策略/安全边界聚合快照；`contextPolicyOperations` v2 已统一输出策略解析顺序、回退行为、审批状态、项目/环境覆盖存储与审批流就绪状态；仍缺按项目/环境的运营后台和策略变更审批。 | 上下文有裁剪、脱敏、inputDigest、来源引用和不泄露原文的策略可观测性。 |
| P0 | 输出 Schema 和质量门禁 | 质量工程师 | 已补候选 JSON Schema、模型原始输出 JSON Schema/parser、生成/编辑落库前质量门禁、重复键和敏感泄露阻断，以及任务报告 `readinessPolicy` 准出阈值运营聚合；仍缺真实 WP2 响应接入联调和可阻断发布流的准出审批。 | 不合格输出不得静默落库；golden set 阈值可回归；运营报表能看到当前准出阈值与风险项。 |
...
| 任务报告上下文装配策略聚合 | 后端任务全量报告新增 `contextAssemblyPolicy` 聚合行，固定输出装配策略版本、`SNAPSHOT_DIGEST_ONLY` 模式、inputDigest 跟踪、仅持久化摘要、仅通过 WP3 应用服务装配、上下文/模型载荷/摘要 digest/正文/schema/页面树/流程 JSON/显式资产 ID/历史步骤不导出等安全标记，以及需求快照组、关联资产组、历史用例组、显式资产类型和裁剪上限计数；不导出原始上下文、显式资产 ID、digest 值、schema、页面树、流程 JSON、需求正文或历史用例步骤，并将专项测试纳入 WP5 quality gate。 |
...
| M5 准出运营 | 可持续评估和发布 | golden set、质量阈值、发布准出记录、观测字段 | 产品经理、质量工程师 | quality gate、smoke、评测报告可复现；新增异步生成专项测试、事件恢复专项测试、生成编排策略报告专项测试、模型观测摘要专项测试、任务报告导出专项测试、任务全量质量摘要/准出阈值专项测试和 `QUEUED` 状态 DB validation。 |
...
| 任务报告摘要被误解为全量准出报告 | “导出报告”已切换为后端全任务聚合，输出任务元数据、生成编排策略聚合字段、模型观测聚合字段、`modelObservationPolicy` 观测边界字段、上下文规模、上下文装配策略与策略治理聚合字段、导出治理聚合字段、导出审计策略聚合字段、安全扫描策略聚合字段、归档策略聚合字段、Prompt 校准策略聚合字段、发布补偿策略聚合字段、报告 manifest 聚合字段、全量候选质量摘要、全量评审历史摘要和发布记录摘要；不输出事件 ID、事件 payload、队列消息体、恢复明细、幂等键、超时错误正文、候选正文、步骤正文、评审评论、请求/响应 preview、模型调用 ID 原值、trace/job 原值、provider 错误正文、受控上下文正文、审计事件明细、候选/trace/审计 ID 清单、安全扫描命中详情、样本明细、归档路径、审批备注、sourceRef、资产用例 ID、显式资产 ID、digest 值、schema、页面树、流程 JSON 或发布错误正文，并写导出审计。 | 后续补真实归档存储、审批流、审计报表模板、样本集维护后台、补偿后台和敏感词扫描运营记录。 |
...
| 导出泄露敏感信息 | 候选服务端筛选导出、评审历史导出、后端任务全量报告导出、已选候选和发布摘要导出均使用白名单字段；候选筛选导出限制 500 条并写审计，任务报告只输出聚合字段并在返回前执行安全扫描；生成编排策略仅输出固定版本、模式、布尔开关和计数，上下文装配策略仅输出固定版本、模式、布尔开关和计数，上下文策略运营 v2 仅输出策略版本、解析顺序、回退行为、审批状态、就绪布尔值和禁止导出标记，导出审计策略仅输出固定动作、资源、项目作用域和布尔开关，安全扫描策略仅输出固定枚举和布尔开关，归档策略仅输出有界数字、布尔值和固定枚举，Prompt 校准策略仅输出版本、来源、状态、计数和布尔开关，发布补偿策略仅输出版本、能力开关和计数，manifest 仅输出版本、行数和完成状态，不导出事件 ID、事件 payload、队列消息体、恢复明细、幂等键、超时错误正文、审计事件明细、候选/trace/审计 ID 清单、安全扫描命中片段、样本行、评审评论、归档路径、审批说明、策略覆盖规则、策略 diff、策略正文、发布 sourceRef、资产用例 ID、显式资产 ID、digest 值、schema、页面树、流程 JSON、发布错误正文、描述、步骤正文、前置条件、原始 Prompt 或完整模型输入。 | 后续将真实归档存储状态、补偿后台状态和敏感词扫描记录纳入统一导出治理。 |
