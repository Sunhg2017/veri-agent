...
| P0 | 输出 Schema 和质量门禁 | 质量工程师 | 已补候选 JSON Schema、模型原始输出 JSON Schema/parser、生成/编辑落库前质量门禁、重复键和敏感泄露阻断、任务报告 `readinessPolicy` 准出阈值运营聚合，`evaluationCorpusPolicy` 评测语料运营策略在 health、任务响应、`contextSummary`、模型 `contextPacking`、任务报告和前端诊断中的聚合快照，`GET /api/v1/test-design/quality/evaluation-corpus-summary` 只读运营摘要会按项目 scope 聚合策略边界、Prompt 版本准出分布和人工反馈样本信号，以及 `releaseReadinessPolicy` 发布准出审批策略聚合快照；`veri-agent.test-design.release-readiness-publish-blocking-enabled=true` 时，正式发布会在写入 WP3 前按任务聚合 readiness=`BLOCKED` 失败关闭，dryRun 仍可用于诊断；release managed HTTP smoke 默认开启该阻断配置，并验证 health 策略快照、dryRun 诊断、阻断重试不追加 publish record 且不写 WP3 用例；`scripts/wp5_quality_gate.sh` 已在发布准出模式强制要求 `WP5_RUN_AI_EVAL=1` 运行 golden set 基线；仍缺真实 WP2 provider 联调、真实样本维护后台、长期校准后台和真实审批工作流/覆盖例外。 | 不合格输出不得静默落库；golden set 阈值可回归且发布准出不可静默跳过；运营报表能看到当前评测语料、准出阈值、默认 advisory-only 或配置化发布阻断边界与风险项。 |
...
| P0 | HTTP smoke 常态化 | 质量工程师 | 已补 managed HTTP smoke runtime，显式开启时可自启动临时 PostgreSQL 和 `platform-api`；smoke 主链路已覆盖项目创建、需求创建、上下文策略覆盖审批、任务 effective snapshot、候选评审、dryRun 和发布；release managed smoke 默认设置 `WP5_RELEASE_READINESS_PUBLISH_BLOCKING_ENABLED=true`，并额外覆盖发布准出策略快照、失败候选形成、readiness=`BLOCKED` 后 dryRun 可诊断、正式重试返回 `INVALID_STATE`、不追加 publish record 且不写 WP3 用例；`scripts/wp5_quality_gate.sh` 已支持 `WP5_GATE_MODE=release`/`WP5_RELEASE_GATE=1` 发布准出模式，发布模式下未启用 HTTP smoke 或未设置 `WP5_RUN_AI_EVAL=1` 会直接失败，`external` 模式缺少 `WP5_SMOKE_BASE_URL` 也会失败；日常 gate 仍避免无意启动 Docker。 | 发布前执行 `WP5_GATE_MODE=release WP5_RUN_HTTP_SMOKE=1 WP5_RUN_AI_EVAL=1 bash scripts/wp5_quality_gate.sh`，或对既有环境执行 `WP5_GATE_MODE=release WP5_RUN_HTTP_SMOKE=external WP5_SMOKE_BASE_URL=... WP5_RUN_AI_EVAL=1 bash scripts/wp5_quality_gate.sh`。 |
...
6. `releaseReadinessPolicy` 能在 health、任务诊断、模型载荷和任务报告中一致说明默认 advisory-only、配置化发布阻断、审批流未就绪、禁止自动发布和候选确认要求；开启发布阻断后，正式发布必须按任务质量 `BLOCKED` 失败关闭且不写入 WP3。
...
| 发布准出策略被误解为真实审批流 | `releaseReadinessPolicy` 默认声明 advisory-only 准出边界；开启 `release-readiness-publish-blocking-enabled` 后只按聚合质量 `BLOCKED` 阻断正式发布，仍固定输出 `approvalWorkflowReady=false`、`qualityGateOverrideSupported=false`、`autoPublishAllowed=false` 和候选确认要求；任务诊断与报告不导出候选级准出证据、审批备注或阈值规则明细。 | 后续补真实审批工作流、质量门禁覆盖例外和运营审批后台。 |
