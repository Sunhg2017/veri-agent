...
| 当前状态 | WP5 已具备事件驱动规则模板生成、候选评审、dryRun、发布到 WP3 的最小闭环；仍未达到企业级产品完成态 |
| 版本 | v0.11 |
| 日期 | 2026-05-27 |
...
| P0 | 任务编排与幂等 | 项目经理、服务端架构师 | 已补 retry/cancel 契约、状态保护、创建任务幂等键、requestDigest 冲突检测、DB 事务锁、`QUEUED -> RUNNING` 条件认领、事件恢复扫描；仍缺运行中超时回收和跨 WP 补偿后台。 | 重试不重复污染候选；取消可阻断排队/运行中任务；重复请求和重复事件可识别。 |
...
| P1 | 前端产品化 | 前端工程师 | 已补任务筛选、候选筛选、候选分页、候选批量评审、批量/发布二次确认摘要、勾选候选发布、冲突摘要、发布结果跳转 WP3 资产追踪、候选编辑字段级质量提示和排队/运行中任务自动刷新；仍需批量字段编辑和服务端分页联动。 | 用户无需 curl 可完成主链路；移动/窄屏无重叠。 |
...
| M4 前端企业化 | 用户在页面完成主流程 | 任务工作台、候选评审、发布预览、错误态 | 前端工程师 | 已具备任务/候选筛选、候选分页、批量评审、批量/发布二次确认摘要、选择性发布、候选质量提示、冲突摘要和 WP3 跳转；仍需批量字段编辑和服务端分页联动。 |
| M5 准出运营 | 可持续评估和发布 | golden set、质量阈值、发布准出记录、观测字段 | 产品经理、质量工程师 | quality gate、smoke、评测报告可复现；新增异步生成专项测试和 `QUEUED` 状态 DB validation。 |
...
| 重试或重复事件导致候选重复 | 重试按 duplicateKey 跳过已有候选；异步事件消费通过 `QUEUED -> RUNNING` 条件认领，重复事件只返回当前任务。 | 继续补运行中超时回收、补偿后台和多实例压测。 |
| 任务停留排队态 | 任务先持久化为 `QUEUED`，提交事件后后台消费；恢复扫描会重发排队事件，HTTP smoke 和前端均按异步状态轮询。 | 后续补队列 lag 指标、超时告警和人工重发入口。 |
...
| 前端发布范围误选 | 发布面板默认覆盖全部可发布候选，勾选候选后仅发布勾选范围；分页候选列表支持页内选择且跨页保留已选计数，发布前展示二次确认摘要。 | 后续增加服务端分页场景下的跨页选择策略。 |
