...
| P1 | AI 质量运营 | 产品经理、质量工程师 | 已补后端任务全量质量摘要、Prompt 版本趋势聚合接口、可配置默认准出阈值摘要、Prompt 版本级准出摘要、Prompt 准出状态分布、前端看板、人工反馈回流聚合和任务报告 `promptCalibrationPolicy` 聚合快照，覆盖任务级和版本级状态/覆盖/优先级分布、可评审/可发布、步骤/最终预期完整性、低置信度、错误、缺需求、缺标题、重复键碰撞、人工修正、驳回/忽略、调优样本说明覆盖计数以及样本维护/长期校准未就绪状态；仍缺真实样本集维护后台和真实 Prompt 变更后的长期趋势校准。 | 每次 Prompt 变更可比较覆盖率、重复率、泄露数、有效步骤率、版本级准出状态和准出状态分布。 |
