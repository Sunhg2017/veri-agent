...
| P0 | 权限和资源作用域加固 | 服务端架构师、质量工程师 | 已补批量候选操作和候选导出按候选/任务项目 scope 鉴权；新增 `scopePolicy` 聚合快照并同步到 health、任务响应、`contextSummary.scopePolicy`、模型 `contextPacking.scopePolicy`、前端任务诊断和任务报告，固定声明任务/候选/批量/发布/异步生成按项目 scope、HTTP smoke 和质量评测项目隔离，以及候选 ID/角色规则/服务令牌原值不导出；新增 `GET /api/v1/test-design/quality/scope-summary` 只读运营摘要，按项目 scope 聚合任务/候选/发布记录项目一致性、作用域覆盖率、模型调用引用和发布作用域记录计数；`evaluationCorpusPolicy` 已单独声明评测语料项目作用域、golden set 基线和导出红线；仍缺完整跨 WP 统一作用域看板和评测语料运营后台。 | 项目角色不能操作其他项目候选；服务令牌调用可审计，运营报表能看到当前 scope、评测语料边界和缺口。 |
...
| 权限与资源作用域策略聚合快照 | 新增 `scopePolicy` 聚合快照，并在 health、任务响应、`contextSummary.scopePolicy`、模型 `contextPacking.scopePolicy`、前端任务诊断和后端任务全量报告中保持同一 scope 边界；固定输出 `PROJECT_RESOURCE_SCOPE`、列表无项目筛选时的平台级 fallback、任务/候选/批量/发布/异步生成/HTTP smoke/质量评测的项目隔离标记，以及评测语料运营后台和跨 WP scope 看板未就绪状态；只读摘要补充任务/候选/发布记录项目一致性、作用域覆盖率、模型调用引用和发布作用域记录聚合；不导出候选 ID 列表、角色规则明细或服务令牌原值，并将专项测试纳入 WP5 quality gate。 |
| 评测语料运营策略聚合快照 | 新增 `evaluationCorpusPolicy` 聚合快照，并在 health、任务响应、`contextSummary.evaluationCorpusPolicy`、模型 `contextPacking.evaluationCorpusPolicy`、前端任务诊断和后端任务全量报告中保持同一评测语料边界；固定输出 `GOLDEN_SET_BASELINE`、`MANUAL_OPT_IN_AI_EVAL`、`DEPLOY_CONFIG` 阈值来源、项目作用域、golden set 基线、AI 评测脚本、质量门禁接入、准出分布与 Prompt 版本跟踪，以及样本维护、长期校准和运营后台未就绪状态；只读运营摘要补充版本准出分布、反馈信号、样本候选和说明覆盖聚合；不导出评测语料行、候选正文、评审评论或 Prompt 正文，并将专项测试纳入 WP5 quality gate。 |
...
| 重试或重复事件导致候选重复 | 重试按 duplicateKey 跳过已有候选；异步生成事件消费通过 `QUEUED -> RUNNING` 条件认领，重复事件只返回当前任务；异步发布事件通过候选级 `PUBLISH_QUEUED -> PUBLISHING` 条件认领，重复事件不会重复创建 WP3 用例或追加发布记录；恢复扫描只重发仍排队的候选。 | 后续补完整运营看板和人工处理台。 |
...
| 发布重复创建 WP3 用例 | 本次已按 `sourceRef=wp5:{candidateId}` 识别已存在用例并链接，新增 AI `sourceRef` 部分唯一索引、迁移前重复数据预检、服务层事务级锁、失败重试补链、受限补偿后台、同需求高相似用例检测、可配置相似度阈值、人工冲突链接入口、异步发布候选级条件认领和资产冲突运营台集中闭环。 | 后续继续补更完整跨 WP 统一运营看板。 |
...
| 发布失败产生部分成功 | 已记录逐候选 publish record；若 WP3 用例已创建但 trace link 缺失，人工重试和受限后台补偿都会按 sourceRef 找回用例并补建链接，候选保留失败原因和资产 ID；异步发布中断会由恢复扫描把超时 `PUBLISHING` 候选转为 `FAILED` 并允许人工重试；任务报告 `publishCompensationPolicy` 会聚合展示回放、补链、重试、后台补偿和人工链接能力边界，但不输出 sourceRef、资产 ID 或错误正文。 | 增加更完整补偿运营台和运行手册。 |
| 作用域策略被误解为权限运营后台 | `scopePolicy` 和 `/quality/scope-summary` 只声明当前项目资源作用域边界、列表 fallback、批量候选项目集合校验、导出禁止项和 aggregate-only 计数；评测语料运营后台与跨 WP 统一 scope 看板显式标记为未就绪。 | 后续补角色/项目/环境级策略运营、跨 WP 审计链 scope 看板和评测语料维护后台。 |
