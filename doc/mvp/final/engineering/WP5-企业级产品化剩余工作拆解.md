...
| P1 | 审计与观测 | 服务端架构师 | 生成、评审、发布已有部分审计；任务响应已补脱敏模型观测摘要，前端诊断面板可查看 prompt/model、`modelInvocationId`、异步 job traceId、成本、延迟、token、fallback、`inputDigest`、`idempotencyKey`、错误摘要、创建/更新时间、上下文键摘要、上下文策略摘要、策略治理状态、策略运营 v2 状态、发布准出审批策略边界、`auditChainPolicy` 跨 WP 审计链策略边界、`archivePolicy` 归档治理策略边界和 `reportManifestPolicy` 报告清单策略边界；`auditChainPolicy` 已同步到 health、任务响应、`contextSummary`、模型 `contextPacking`、任务诊断和任务报告，声明当前只做 WP5 本域聚合与 WP1 审计写入/WP2 调用/WP3 发布引用追踪，不导出审计事件明细、候选 ID 清单、平台审计标识原值、traceId 原值、模型调用 ID 原值、发布 sourceRef 或资产 ID 原值；`reportManifestPolicy` 已同步到 health、任务响应、`contextSummary`、模型 `contextPacking`、任务诊断和任务报告，声明当前只做 schema/字段集/行数/完成状态聚合核验，不导出行级完整性值、行内容摘要、候选 ID、trace ID 或审计 ID 清单；任务报告已补 `modelObservationPolicy` 聚合行声明模型观测只导出路由、成本、延迟和 token 聚合信号，不导出 trace/job/invocation ID 原值、载荷预览或 provider 错误正文；任务报告已补本域审计链摘要，聚合 WP5 任务、评审记录和发布记录的事件数、说明覆盖、失败/冲突和最近事件；任务报告已补 `auditPolicy` 聚合行声明导出审计写入策略；仍缺跨 WP1 audit_log 的端到端统一审计链看板和 audit outbox 重放看板。 | 任务详情能定位模型调用、错误原因、上下文策略、发布准出边界、审计链和报告清单核验边界。 |
| P1 | 导出和报告摘要 | 产品经理、前端工程师 | 已补服务端按筛选条件的候选 CSV 导出、导出审计、前端筛选导出、已选候选、发布结果摘要导出、评审历史页摘要、后端任务全量报告导出、生成编排策略聚合行、模型观测聚合行、`modelObservationPolicy` 聚合行、上下文策略聚合行、`contextAssemblyPolicy` v2 聚合行、`contextPolicyOperations` 聚合行、`scopePolicy` 聚合行、`evaluationCorpusPolicy` 聚合行、`releaseReadinessPolicy` 聚合行、`auditChainPolicy` 聚合行、`readinessPolicy` 聚合行、`exportGovernance` 聚合行、`auditPolicy` 聚合行、`safetyScanPolicy` 聚合行、`archivePolicy` 聚合行、`reportManifestPolicy` 聚合行、`promptCalibrationPolicy` 聚合行、`publishCompensationPolicy` 聚合行、`reportManifest` 聚合行和报告最终安全扫描；仍缺真实归档存储、归档审批流、报告行级完整性存储和更完整的审计报表。 | 导出不含事件 ID、事件 payload、队列消息体、恢复明细、幂等键、超时错误正文、原始 Prompt、密钥、token、受控上下文正文、模型调用 ID 原值、job/trace 原值、模型载荷预览、provider 错误正文、审计事件明细、候选/trace/审计 ID 清单、平台审计标识原值、安全扫描命中详情、评测语料行、样本行、候选正文、评审评论、归档路径、审批备注、策略覆盖规则、策略变更说明、工单 URL、候选级准出证据、阈值规则明细、发布候选 ID、资产用例 ID、sourceRef、trace 明细、行级完整性值、行内容摘要或发布错误正文，命中敏感模式时失败关闭。 |
...
10. 重复发布、重复候选和同需求高相似用例不会静默污染资产库。
11. 前端能覆盖 loading、empty、error、403、409、partial success、traceId 和发布结果。
12. `mvn -B -pl platform-api test`、`cd portal-web && npm test`、`cd portal-web && npm run build` 与 WP5 专项 gate 按影响面执行并记录。
...
| 报告清单策略被误解为明细索引 | `reportManifestPolicy` 只声明当前聚合核验边界，固定输出 schema/字段集版本、行数/完成状态跟踪、归档核验 ready、明细导出关闭和 `aggregateOnly=true`；任务诊断与报告不导出行级完整性值、行内容摘要、候选 ID、trace ID 或审计 ID 清单。 | 后续如需行级完整性存储，必须先补存储、权限、脱敏、审批和导出安全设计。 |
...
| 导出泄露敏感信息 | 候选服务端筛选导出、评审历史导出、后端任务全量报告导出、已选候选和发布摘要导出均使用白名单字段；候选筛选导出限制 500 条并写审计，任务报告只输出聚合字段并在返回前执行安全扫描；生成编排策略仅输出固定版本、模式、布尔开关和计数，作用域策略仅输出固定 scope 模型、项目隔离标记和禁止导出标记，评测语料策略仅输出固定版本、模式、阈值来源、能力开关和运营缺口，发布准出审批策略仅输出固定决策模式、阈值来源、布尔边界和聚合计数，审计链策略仅输出固定版本、模式、事件来源、布尔边界和事件聚合计数，上下文装配策略仅输出固定版本、模式、布尔开关和计数，上下文策略运营 v2 仅输出策略版本、解析顺序、回退行为、审批状态、就绪布尔值和禁止导出标记，导出审计策略仅输出固定动作、资源、项目作用域和布尔开关，安全扫描策略仅输出固定枚举和布尔开关，归档策略仅输出有界数字、布尔值和固定枚举，报告清单策略仅输出版本、schema、字段集、模式、行数/完成状态跟踪和禁止导出标记，Prompt 校准策略仅输出版本、来源、状态、计数和布尔开关，发布补偿策略仅输出版本、能力开关和计数，manifest 仅输出版本、行数和完成状态，不导出事件 ID、事件 payload、队列消息体、恢复明细、幂等键、超时错误正文、审计事件明细、候选/trace/审计 ID 清单、平台审计标识原值、安全扫描命中片段、评测语料行、样本行、评审评论、归档路径、审批说明、策略覆盖规则、策略 diff、策略正文、候选级准出证据、阈值规则明细、发布 sourceRef、资产用例 ID、显式资产 ID、digest 值、schema、页面树、流程 JSON、候选 ID 列表、角色规则明细、服务令牌原值、行级完整性值、行内容摘要、发布错误正文、描述、步骤正文、前置条件、原始 Prompt 或完整模型输入。 | 后续将真实归档存储状态、补偿后台状态、跨 WP scope/审计链看板、报告行级完整性存储和敏感词扫描记录纳入统一导出治理。 |
