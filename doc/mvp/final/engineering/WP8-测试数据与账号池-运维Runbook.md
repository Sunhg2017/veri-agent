...
| 当前口径 | WP8 当前提供控制面、租借并发保护、清理任务记录和脱敏导出摘要；默认不执行真实 cleanup worker 或破坏性清理 adapter |
| 日期 | 2026-06-15 |

## 1. 适用范围

本 Runbook 适用于 WP8 开发、预发和生产发布准出，以及账号租借卡死、账号锁定、SecretRef 轮换、清理失败和脱敏导出异常排障。WP8 当前仍由 `platform-api` 承载测试数据与账号池控制面，真实凭据解析和真实清理执行不在当前默认能力内。
...
1. 首次发布先保持 `cleanup-enabled=false`，确认控制面、租借和导出稳定后，再对真实清理 adapter 做独立专项准出。
...
5. `cleanup-enabled=false` 时不得执行破坏性清理动作，只允许记录和重试控制面任务。
6. 数据集导出不得包含完整 record payload、maskedSummary 值、`secretRef` 原文、token、cookie 或 Authorization header。
7. 租借导出不得包含租借 token 明文、释放原因原文、健康摘要原文、scopeSummary 值、leasePolicy 值或敏感 key。
...
| 真实 adapter 后续启用失败 | 立即关闭 `cleanup-enabled`，保留任务、adapter 日志和审计，回到控制面只读记录。 | 配置变更、失败任务列表、回滚记录。 |

当前默认不执行破坏性清理动作，因此“任务创建成功”不等于“外部系统已清理”。任何真实清理 adapter 必须先具备 allowlist、dry-run、幂等、最大影响范围和回滚策略。
...
| 导出结果出现 `secret://`、token、cookie、Authorization 或原文释放原因 | 立即关闭 `export-enabled`，撤销相关导出权限，修复后端 view 和前端 normalizer，重跑 WP8 quality gate。 |

导出是审计摘要，不是数据搬运工具。后续如果实现文件下载，必须沿用当前 redaction policy 和字段白名单，并新增文件级 smoke 与泄露扫描。
...
3. 暂停新租借：禁用对应账号池，或将高风险账号置为 `LOCKED/DISABLED`。
4. 保留数据集、账号池、账号、租借、清理任务和审计记录；不要直接删除证据。
5. 修复后重跑 `WP8_GATE_MODE=release WP8_LEASE_CONCURRENCY_SMOKE=managed bash scripts/wp8_quality_gate.sh`。
6. 对涉及数据库或权限 seed 的回滚，追加 `bash db/validation/run_wp1_db_validation.sh`。

## 11. 准出记录

发布、账号池接入、SecretRef 轮换或真实清理专项工单至少记录：
