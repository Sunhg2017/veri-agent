...
| 后续项 | 当前判断 | 不阻断原因 |
|---|---|---|
| 真实文件下载 | 后续增强 | 当前导出是控制面 JSON 摘要，已满足脱敏审计视图。 |
| 真实 cleanup worker / 破坏性 adapter | 后续安全专项 | 当前默认 `cleanup-enabled=false`，只记录任务，避免误删业务数据。 |
| WP7 UI/E2E runner 凭据注入 | 进行中 | WP8 已提供 runner contract 和 runner-only 凭据解析；当前 preview adapter 已生成脱敏注入计划摘要，但真实浏览器执行与站点登录仍待 WP7 执行器落地。 |
| WP9 调度自动申请/释放 | 已由 WP9 接入 | WP8 仍拥有账号池、租借状态机和脱敏契约；WP9 只在运行时申请/释放并保存安全摘要，不阻断 WP8 准出。 |
| WP10 完整报告生成 | 后续 WP10 | WP8 已提供准备、租借和清理证据摘要，不生成报告正文。 |
