...
截至 WP8 文件下载、XXL-JOB 触发的 cleanup worker/HTTP adapter 和真实业务账号自动开通 HTTP adapter 切片收口，WP8 当前承诺范围内没有剩余 P0 功能开发项。剩余工作只剩发布前按目标环境执行 release gate、填写发布记录，以及后续增强另行立项。
...
| 账号池 | 账号池 CRUD、禁用/归档、账号摘要新增/更新、`secretRefDigest`、健康摘要、HTTP 自动开通 adapter 和不回显 secretRef。 |
...
| 真实 cleanup worker / 破坏性 adapter | 已完成受控实现 | 当前默认 `cleanup-enabled=false`；XXL-JOB 触发 worker，显式开启且 HTTP adapter ready 后才执行真实清理，失败和摘要均可审计。 |
| WP7/WP9/WP10 对 WP8 契约的真实消费 | 已由对应 WP 接入 | WP8 继续拥有账号池、租借状态机和脱敏契约；WP7 凭据注入、WP9 自动申请/释放和 WP10 报告证据消费均已通过应用层契约接入，不再构成 WP8 剩余项。 |
| 真实业务账号自动开通 | 已完成 HTTP adapter 实现 | 当前默认 `account-provisioning-enabled=false` 且 adapter mode 为 `DISABLED`；生产开通要求 HTTP adapter ready，按 `leasePolicy.provisioning`、`minAvailable/maxAccounts` 和 batch size 补齐账号；`LOCAL_SECRET_REF` 仅为本地演练模式。 |
...
| 多实例 worker 运维演练 | 后续运维专项 | 当前 WP8 worker 由 XXL-JOB 触发，并通过方法同步、requestKey/accountKey 幂等和 repository 条件更新避免重复处理；多实例故障切换和锁指标需目标环境演练。 |
