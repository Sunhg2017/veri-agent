...
| 当前口径 | 当前范围已推进至 M8E：`platform-api` 已新增 `reporting` 模块，消费 WP9/WP8 脱敏摘要，通过 WP2 受控模型调用生成诊断建议，并提供缺陷草稿、JSON/Markdown 脱敏导出和 quality gate；WP3/WP5 evidence adapter 保持后续专项 |
| 版本 | v0.2 |
| 日期 | 2026-06-17 |
...
flowchart LR
    ...
    EVIDENCE -. 后续专项 .-> WP3["WP3 Asset Summary Port"]
    EVIDENCE -. 后续专项 .-> WP5["WP5 Manifest Port"]
...
| `ReportEvidenceService` | 当前聚合 WP9/WP8 证据 manifest；WP3/WP5 port 保持后续专项，不保存原始 evidence 正文。 |
...
| `GET` | `/{id}` | `report:read` | M5B 已实现。查询报告详情并返回 WP9/WP8 evidence manifest、最新诊断和缺陷草稿摘要；WP3/WP5 证据保持后续专项。 |
