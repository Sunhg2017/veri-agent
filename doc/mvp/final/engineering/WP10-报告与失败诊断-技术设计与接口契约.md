...
6. `report_export_manifest(report_id,export_type,exported_at)`。
...
| 方法 | 路径 | 权限 | 说明 |
|---|---|---|---|
| `GET` | `/health` | `report:read` | 返回开关、limits、schema 和 redaction policy 摘要。 |
| `POST` | `/` | `report:generate` | 基于 executionRunId 生成报告快照。 |
| `GET` | `/` | `report:read` | 分页查询报告列表。 |
| `GET` | `/{id}` | `report:read` | 查询报告详情、证据 manifest、诊断摘要和草稿摘要。 |
| `POST` | `/{id}/retry` | `report:generate` | 对 FAILED 报告重试生成。 |
| `POST` | `/{id}/archive` | `report:manage` | 归档报告。 |
...
```json
{
  ...
  "redactionPolicy": {
    ...
    "requestResponseBodyExported": false
  },
  "traceId": "trc_xxx"
}
```
