...
| 当前口径 | 当前范围已推进至 M12：`platform-api` 已新增 `reporting` 模块，消费 WP9/WP8/WP3/WP5 脱敏摘要，支持默认同步生成和可选异步生成 worker，通过既有快照提供同项目报告对比，通过 WP2 受控模型调用生成诊断建议，并提供缺陷草稿、JSON/Markdown/PDF/Word 脱敏导出、真实文件下载、报告完成 webhook 回调和 quality gate |
| 版本 | v0.3 |
| 日期 | 2026-06-17 |
...
flowchart LR
    ...
    DRAFT --> REPO
    EXPORT --> REPO
    REPORT --> EVENT["PlatformEventPublisher"]
    EVENT --> WEBHOOK["ReportingWebhookDispatcher"]
...
| `ReportExportController` | JSON/Markdown/PDF/Word 脱敏报告和 manifest 导出。 |
...
| `ReportExportService` | 生成导出报告、字段集版本、digest、下载文件和 redaction scan 结果。 |
| `ReportingEventPublisher` | 在报告到达终态后发布内部平台事件，解耦 webhook 回调与报告事务。 |
| `ReportingWebhookDispatcher` | 读取终态报告快照并异步投递 aggregate-only webhook；失败仅写告警和审计。 |
...
| `report_export_manifest` | `id/report_id/export_type/status/schema_version/field_set_version/redaction_policy_json/content_digest/aggregate_only/exported_by/exported_at/block_reason` | JSON/Markdown/PDF/Word 导出 manifest。 |
...
| `veri-agent.reporting.webhook-delivery-enabled` | `false` | 是否启用报告完成 webhook 回调。 |
| `veri-agent.reporting.webhook-delivery-url` | 空 | 全局 webhook callback URL，仅支持 `http/https`。 |
| `veri-agent.reporting.webhook-delivery-signature-enabled` | `false` | 是否附带 `X-VA-*` HMAC 签名头。 |
| `veri-agent.reporting.webhook-delivery-secret-ref` | 空 | webhook HMAC secretRef，通过 `SecretProvider` 按项目上下文解析。 |
| `veri-agent.reporting.webhook-delivery-timeout-ms` | `5000` | 单次回调投递超时，服务端会限制到安全范围。 |
...
| `report:export` | JSON/Markdown/PDF/Word 报告导出。 |
...
| `report.webhook.delivered` | 终态回调投递成功或失败 | reportId、projectId、terminalStatus、callbackUrlDigest、signatureEnabled、failureReason(仅失败) |

审计 payload 不记录 actor 之外的个人标识值、原始 payload、Prompt、响应正文、stdout/stderr、secretRef 明文、token、cookie 或 Authorization。

M10 webhook 实现说明：报告到达 `READY/FAILED` 终态后，`ReportService` 通过 `ReportingEventPublisher` 在事务提交后发布 `report.webhook.delivery.requested` 内部事件，`ReportingWebhookDispatcher` 再异步读取最新快照并 POST 到配置的 callback URL。payload 仅包含 `reportId/projectId/executionRunId/status/terminalStatus/schemaVersion/sourceRunDigest/summary/generatedAt/latestDiagnosis/redactionPolicy` 等 aggregate-only 字段，不包含 raw evidence、runner stdout/stderr、请求响应正文、webhook payload 回显、prompt/response 原文、secret/token/cookie/Authorization。签名开启时，dispatcher 使用 `SecretResolveContext("WEBHOOK_SIGNING", "wp10-reporting-service", "PROJECT", projectId)` 解析 secret，并附加 `X-VA-Timestamp`、`X-VA-Event-Id`、`X-VA-Signature-Algorithm`、`X-VA-Signature`。回调失败只写告警和 `report.webhook.delivered` 失败审计，不回滚报告终态。
...
| `GET` | `/{id}/export` | `report:export` | M12 已实现。`exportType=JSON|MARKDOWN|PDF|WORD` 导出 aggregate-only 脱敏报告和 manifest；只允许 READY 报告，数据库仅保存 export manifest、content digest 和 redaction policy，不保存导出正文。 |
| `GET` | `/{id}/exports/{exportId}/download` | `report:export` | M12 已实现。下载已物化的 PDF/Word/JSON/Markdown 导出文件，仍通过 manifest 和 opaque storage 边界控制。 |
...
M12 实现说明：`GET /api/v1/reports/{id}/export?exportType=JSON|MARKDOWN|PDF|WORD` 会校验 reporting/export 开关、`report:export` 项目 scope、报告存在性和 READY 状态。导出内容即时从 `report_execution_report`、`report_evidence_manifest`、最新 `report_failure_diagnosis` 和 `report_defect_draft` 的聚合快照生成，只包含 report snapshot、summary、evidence manifest 摘要、latestDiagnosis、defect draft 摘要和 redaction policy；不包含 runner stdout/stderr、请求响应正文、webhook payload、raw prompt/raw response、provider payload、secret/token/cookie/Authorization、`secret://` 原文、账号 lease token 或跨 WP 原始 ID 清单。JSON/Markdown 继续返回可预览内容，PDF/Word 由 `ReportDocumentRenderer` 生成真实文档字节，通过既有 `report_export_manifest + opaque storage + /download` 通道供下载。服务端会过滤高风险 key、脱敏文本并扫描导出文本视图；命中禁止文本时写入 `report_export_manifest.status=BLOCKED/blockReason=REPORT_EXPORT_REDACTION_BLOCKED` 并返回空 `content`。成功时写入 `CREATED` manifest、`contentDigest`、`schemaVersion`、`fieldSetVersion`、`aggregateOnly=true`、`exportedBy/exportedAt` 和 redaction policy，报告 summary 同步更新 `exportManifestCount`。导出正文不落库，不写外部缺陷系统。health 中 `exportSummaryReady=true`。
