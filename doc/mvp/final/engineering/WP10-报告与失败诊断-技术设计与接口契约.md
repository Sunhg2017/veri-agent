...
| `GET` | `/{id}` | `report:read` | M4A 已实现。查询报告详情并返回 WP9 节点级 evidence manifest、已识别 WP8 引用的 aggregate-only manifest 和最新 `RULE_READY` 规则诊断；WP3/WP5 证据、WP2 AI 诊断和草稿仍返回空结构或关闭态。 |
...
| `POST` | `/{id}/diagnoses` | `report:diagnose` | 触发 AI 诊断或回放既有诊断。 |
| `GET` | `/{id}/diagnoses/latest` | `report:read` | 查询最新诊断详情。 |
...
M4A 当前响应口径：在 AI 诊断接入前，`latestDiagnosis.status` 固定为 `RULE_READY`，`classificationOnly=true`、`modelInvoked=false`、`aiDiagnosisReady=false`，`modelInvocationDigest` 为空；`rootCauseCandidates[*].evidenceRefs` 仅包含 `wp9:execution_node:<digest>` 或 `wp8:account_lease:<digest>` 这类 digest 引用，不包含源节点 ID、账号租借 ID 或账号凭据。
