...
| 当前口径 | 用户可在 `#test-data` 工作台完成数据集、账号池、账号摘要、租借、释放、清理任务、脱敏导出摘要和导出文件下载主链路，不依赖 curl |
| 日期 | 2026-06-24 |

## 1. 适用范围

本文面向测试工程师、自动化工程师、项目负责人和审计人员，说明如何通过 `portal-web` 的 `测试数据` 工作台完成 WP8 当前已交付的浏览器操作。本文不替代运维排障 Runbook；真实 cleanup adapter 和业务账号自动开通属于显式配置能力，默认保持关闭或受控模式，WP7/WP9/WP10 对 WP8 契约的消费已在对应工作包交付。
...
| 生成或下载数据集/租借脱敏导出文件 | `testData:export` |
...
| 数据集 tab | 创建/保存/归档数据集，导入记录摘要，查看记录 digest，生成数据集脱敏导出摘要并下载 JSON 文件。 |
| 账号池 tab | 创建/保存/禁用/归档账号池，新增或编辑账号摘要，替换 `secretRef`。 |
| 租借 tab | 申请租借、续租、释放账号，查看 lease token digest，生成租借脱敏导出摘要并下载 JSON 文件。 |
...
5. 如需留存文件，点击 `下载文件`，浏览器会下载与当前脱敏摘要 schema 一致的 JSON attachment。
6. 导出摘要和下载文件都不展示 maskedSummary 值、完整记录正文、`secretRef` 原文、token、cookie 或 Authorization header。
...
8. 如果运维已启用账号自动开通，账号池 `leasePolicy JSON` 可包含 `provisioning` 配置；页面仍只展示生成后的账号摘要和 `secretRefDigest`，不会展示 adapter 返回的 `secretRef` 原文。
...
5. 如需留存文件，点击 `下载文件`，浏览器会下载与当前租借脱敏摘要 schema 一致的 JSON attachment。
6. 导出摘要和下载文件都不展示 `secretRef` 原文、租借 token 明文、释放原因原文、账号健康摘要原文、scopeSummary 值、leasePolicy 值、token、cookie 或 Authorization header。

## 9. 清理任务操作

1. 选择 `清理任务` tab。
2. 查看表单说明：`cleanupEnabled=false` 时，当前环境只记录控制面任务，不执行破坏性清理动作；显式开启且 adapter ready 后，worker 才会调用受控 cleanup adapter。
...
| 清理任务没有真实执行 | 查看策略区 `清理执行`，再由运维检查 health policy 中的 cleanup adapter ready/provider；默认 `cleanupEnabled=false` 只记录控制面任务。 |
| 导出按钮置灰 | 检查 `testData:export` 权限和 `exportEnabled` 开关。 |
| 下载按钮失败 | 检查同一对象的 `导出摘要` 是否可成功、浏览器是否拦截下载、接口是否返回 traceId 和 `testData:export` 权限。 |
| 页面出现敏感原文 | 立即停止发布或验收，保留 traceId 和截图，重跑 WP8 Playwright smoke 与脱敏测试。 |

## 12. 产品验收清单

1. 用户可不依赖 curl 完成数据集创建、更新、归档、记录摘要导入、数据集脱敏导出摘要查看和文件下载。
2. 用户可不依赖 curl 完成账号池创建、更新、禁用、归档和账号摘要维护。
3. 用户可不依赖 curl 完成账号租借、续租、释放、租借脱敏导出摘要查看和文件下载。
4. 用户可不依赖 curl 创建和重试清理任务控制面记录，并理解 `cleanupEnabled=false` 和 adapter ready 的安全含义。
