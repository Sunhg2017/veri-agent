...
本文面向测试工程师、自动化工程师、项目负责人和审计人员，说明如何通过 `portal-web` 的 `测试数据` 工作台完成 WP8 当前已交付的浏览器操作。本文不替代运维排障 Runbook，也不承诺真实文件下载、真实 cleanup worker、WP7/WP9 真实执行器或 SecretProvider 凭据注入；这些能力仍需独立准出。
