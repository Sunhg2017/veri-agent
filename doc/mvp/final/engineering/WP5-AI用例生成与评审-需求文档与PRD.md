...
| 角色产出 | 资深产品经理 |
| 文档性质 | 需求文档与产品 PRD |
| 当前口径 | 基于 WP4 已发布需求资产、WP3 资产库和 WP2 模型接入生成可评审测试用例；任务诊断和任务报告仅以聚合方式披露生成编排策略、权限与资源作用域策略、评测语料运营策略、发布准出审批策略、跨 WP 审计链策略、跨 WP 审计链只读聚合骨架、跨 WP 统一运营看板、细粒度队列告警订阅、人工 queued generation/publish 重放面板、发布补偿运行手册、批量运营审计报表、归档治理策略、报告清单策略、上下文装配策略 v2、上下文策略治理、上下文策略运营 v2 状态、项目/环境上下文策略覆盖后端元数据、审批状态、effective policy 快照、模型观测策略和质量/导出治理状态 |
| 版本 | v0.15 |
| 日期 | 2026-06-11 |
...
| 跨 WP 运营 | 支持在统一运营面板维护细粒度队列告警订阅、触发有界 queued generation/publish 重放、查看发布补偿运行手册和批量运营审计报表；所有输出保持 aggregate-only，不暴露任务 ID、候选 ID、outbox payload、traceId、模型调用 ID、sourceRef 或 WP3 资产 ID。 | 运营人员可基于聚合计数发现排队、滞留、超时和补偿风险，并按项目/prompt 做有界操作。 |
...
| 跨 WP 审计链看板 | 当前只暴露 `auditChainPolicy` 聚合快照和 `/report/audit-chain` 任务级只读聚合骨架；不提供跨 WP1 audit_log、WP2 调用和 WP3 发布记录的明细级统一审计看板，但已提供跨 WP 统一运营看板、队列告警订阅、人工重放、补偿运行手册和批量运营审计报表。 |
...
| A15 | 统一运营面板可维护细粒度队列告警订阅、触发有界 queued generation/publish 重放、查看发布补偿运行手册和批量运营审计报表；响应和报告始终保持 aggregate-only，不导出任务 ID、候选 ID、outbox payload、traceId、模型调用 ID、sourceRef 或 WP3 资产 ID。 |
