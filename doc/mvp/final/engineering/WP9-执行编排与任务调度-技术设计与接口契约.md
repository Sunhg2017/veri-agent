...
M8B 增加 Scheduler 与 Trigger Runbook，不改变运行时契约。`WP9-Scheduler-Trigger-Runbook.md` 记录 scheduler/webhook/cron 开关、release gate、claim recovery、requestKey/sourceEventId 重放、webhook secret 轮换、CRON 暂停/变更/错过 fire 策略、排障表和回滚顺序。
...
M8F 增加 CRON 容量策略准出 smoke，不改变服务端接口契约。`scripts/wp9_cron_capacity_smoke.sh` 验证长时间积压的 due `nextFireAt` 在一次 tick 中只 materialize 一次，`nextFireAt` 会推进到当前 scan time 之后，重复 tick 不会补偿历史窗口。当前策略仍然明确不做 missed-fire backfill；如需补偿历史窗口，应另行设计 backfill 限额和速率控制。

M8G 增加 CRON backlog 批次准出 smoke，不改变服务端接口契约。`scripts/wp9_cron_backlog_smoke.sh` 验证多个 due CRON trigger 同时积压且超过 `schedulerTickBatchSize` 时，单次 tick 只扫描和触发 batch 上限内的 trigger；未扫描的 due trigger 不生成 trigger event 或 run，并由后续 tick 接续处理。该 smoke 是本地控制面批次边界验证，不替代真实生产压测。
...
M1、M2、M3A、M3B、M3C、M3D、M4A、M4B、M4C、M5、M6A、M6B、M6C、M7A、M7B、M7C、M7D、M8A、M8B、M8C、M8D、M8E、M8F、M8G 和 M8H 已完成：

1. 权限、DB、health、plan CRUD、DAG validator、plan dry-run、手动触发、run/node run 初始化、取消、控制面重试、内部 queue claim、claim heartbeat、过期 claim recovery、节点完成回传、API_TEST 到 WP6 应用服务 dispatch、`baseUrlRef` 环境解析、计划 `runtimeSecretRefs` 安全中继、后台 scheduler loop、webhook/cron 触发控制面、生产 CRON scanner、run export、WP10 report handoff 准出 smoke、CRON 容量 smoke、CRON backlog 批次 smoke、前端主链路、前端操作说明、Playwright smoke、managed scheduler smoke、webhook HTTP smoke、CI webhook 签名样例、scheduler/trigger runbook、供应商 marketplace 接入包、worker 托管 readiness、依赖推进和 run 聚合。
...
4. WP10 完整报告消费集成、报告生成和 WP10 端到端发布准出。
