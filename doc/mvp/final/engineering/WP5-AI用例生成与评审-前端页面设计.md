...
| `fetchTestDesignQueueAlertSubscriptions(filters)` | 查询队列告警订阅。 |
| `upsertTestDesignQueueAlertSubscription(payload)` | 创建或更新队列告警订阅。 |
| `replayTestDesignQueuedEvents(payload)` | 有界重放 queued generation/publish 事件。 |
