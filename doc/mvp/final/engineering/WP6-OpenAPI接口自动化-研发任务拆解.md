...
| WP6-6.2 Runner 配置 | 已完成 | 新增 runner-enabled、timeout、maxCases、allowlist、artifact size 配置边界；`runner-enabled=true` 时装配 Managed HTTP adapter，health 返回安全策略摘要。 |
| WP6-6.3 Run API | 已完成 | 新增 `POST /runs`、`GET /runs/{id}` 和 `POST /runs/{id}/cancel`，按脚本包/run 项目 scope 校验 `apiAutomation:execute/read`；未审批脚本包阻断，runner disabled 返回稳定 `RUNNER_DISABLED` 运行摘要，终态 run 取消幂等返回当前摘要。 |
...
| WP6-6.5 结果采集 | 已完成 | 保存 run status、runnerMode、errorCode/errorSummary 和 case-level status/duration/assertionSummary；不保存完整 request/response/stdout/stderr 或 baseUrl 明文。 |
...
| WP6-6.6 Timeout/cancel | 部分完成 | Managed HTTP adapter 已按请求超时返回 `TIMEOUT/RUNNER_TIMEOUT`；控制面已暴露 `POST /runs/{id}/cancel`，只对 `QUEUED/RUNNING` 调用 runner port 并在 adapter 接受时标记 `CANCELED/RUNNER_CANCELED`，终态 run 幂等返回；异步 cancel smoke 待后续调度型 runner 后补。 |
...
| WP6-7.7 运行面板 | 已完成 | 工作台新增已审批脚本包的 baseUrl/environment/caseIds 运行入口，按 `apiAutomation:execute` 控制，展示 runner 策略、host/digest、状态、错误码和结果聚合；`QUEUED/RUNNING` run 可触发取消；按 `apiAutomation:export` 支持脱敏运行摘要导出。 |
...
| WP6-8.2 前端测试 | 部分完成 | 已覆盖 API helper、权限 helper、生成任务 normalize、脚本包 normalize、run normalize、run cancel/export normalize、生成 API、评审 API、run API、run cancel API 和 run export API 调用，并纳入 `scripts/wp6_quality_gate.sh`；复杂页面交互待后续 Playwright smoke。 |
