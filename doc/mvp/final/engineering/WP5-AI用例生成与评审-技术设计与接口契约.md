...
flowchart LR
    ...
    API --> OPS["TestDesignCrossWpOperationsService"]
...
| `TestDesignTaskReportService` | 导出任务级聚合报告，提供任务本域审计链摘要，并在最终安全扫描通过后保存 aggregate-only manifest 与托管归档内容，触发行级完整性索引和初始归档审批工单，避免报告拼装逻辑回流到任务服务。 |
| `TestDesignReportArchiveService` | 管理任务报告归档元数据、行级完整性摘要、归档审批、外发审批和工单备注时间线；只返回聚合元数据和审批状态，不导出归档正文、内部存储 key 或行 digest。 |
...
| `TestDesignCrossWpOperationsService` | 提供跨 WP 统一运营看板、队列告警订阅 CRUD、人工 queued generation/publish 重放、补偿运行手册、批量运营审计报表、审计报表模板、模型观测聚合钻取和跨 WP 脱敏明细审计报表；只返回聚合计数、脱敏桶和有界操作结果，不暴露任务/候选/outbox/trace/model invocation 明细标识。 |
| `TestDesignScopeService` | 为权限解析提供任务/候选项目作用域，不承载业务流。 |
| `TestDesignRepository` | 维护生成任务、候选、评审记录、发布记录、报告 manifest 聚合记录、归档内容、行级完整性索引、归档审批工单和任务级跨 WP 审计链聚合只读查询。 |
...
### 3.1.2 test_design_queue_alert_subscription

`test_design_queue_alert_subscription` 保存细粒度队列告警订阅，只允许存储项目/prompt scope、告警类型、渠道、bounded targetRef、阈值秒数、启停状态和审计元数据，不允许存储 webhook URL、token、payload、任务 ID、候选 ID 或 outbox 明细。

| 字段 | 类型 | 说明 |
|---|---|---|
| `id` | uuid | 订阅记录 ID |
| `project_id` | varchar(64) | 所属项目 |
| `prompt_key` | varchar(128) | 可选 prompt scope；为空表示项目级订阅 |
| `alert_type` | varchar(64) | `GENERATION_QUEUE_LAG`、`GENERATION_TIMEOUT`、`PUBLISH_QUEUE_LAG`、`PUBLISH_TIMEOUT`、`COMPENSATION_FAILURE`、`AUDIT_OUTBOX_REPLAY_ELIGIBLE` |
| `channel` | varchar(32) | `OPS_CONSOLE`、`EMAIL` 或 `WEBHOOK` |
| `target_ref` | varchar(180) | 非密钥的渠道目标引用 |
| `threshold_seconds` | int | 队列滞留/超时阈值 |
| `enabled` | boolean | 是否启用 |
| `created_by` / `updated_by` | varchar(128) | 审计元数据 |
| `created_at` / `updated_at` | timestamptz | 时间戳 |

约束与索引：

- `uk_test_design_queue_alert_subscription(project_id, coalesce(prompt_key,''), alert_type, channel, target_ref)` 保证稳定 upsert。
- `idx_test_design_queue_alert_subscription_scope(project_id, prompt_key, alert_type, enabled)` 支持运营查询。
- `ck_test_design_queue_alert_subscription_target_ref` 约束 targetRef 仅允许有界非敏感引用。

### 3.1.3 test_design_operations_audit_aggregate

`test_design_operations_audit_aggregate` 只作为运营读模型，不单独存行。它聚合 WP5 批量运营动作的审计计数，范围限定为队列告警订阅变更、queued replay、发布补偿运行和 audit outbox requeue，不导出任何审计明细、actor、trace 或 payload。

| 字段 | 类型 | 说明 |
|---|---|---|
| `total_operation_count` | bigint | 总操作数 |
| `success_count` | bigint | 成功数 |
| `failed_count` | bigint | 失败数 |
| `denied_count` | bigint | 拒绝数 |
| `queue_alert_subscription_mutation_count` | bigint | 队列告警订阅变更数 |
| `queued_event_replay_count` | bigint | queued replay 次数 |
| `publish_compensation_run_count` | bigint | 手工补偿运行次数 |
| `audit_outbox_requeue_count` | bigint | audit outbox 重排队次数 |
| `latest_operation_at` | timestamptz | 最近操作时间 |
...
`test_design_report_archive` 保存安全扫描通过后的托管归档正文、归档摘要、状态和保留截止时间。该表允许保存 `content_bytes` 作为平台内部托管正文，但 API、任务诊断、报告 CSV 和前端不得返回正文、内部存储 key 或路径。

| 字段 | 类型 | 说明 |
|---|---|---|
| `id` | uuid | 归档记录 ID |
| `manifest_id` | uuid | 对应 `test_design_report_manifest` |
| `task_id` / `project_id` | uuid / varchar(64) | 任务和项目 scope |
| `storage_backend` | varchar(32) | 当前固定 `DB_MANAGED` |
| `content_digest` | varchar(64) | 安全扫描后 CSV SHA-256 |
| `content_bytes` | bytea | 内部托管正文，不经 API 导出 |
| `content_size_bytes` | bigint | 正文字节数 |
| `report_row_count` / `line_integrity_count` | bigint | 报告行数和完整性索引行数 |
| `status` | varchar(32) | `PENDING_APPROVAL`、`ARCHIVED`、`REJECTED` |
| `archive_approval_status` / `external_approval_status` | varchar(32) | 归档审批和外发审批状态 |
| `retention_until` | timestamptz | 保留截止时间 |

`test_design_report_archive_line_integrity` 保存行级链式完整性索引，只保存 `row_number`、`row_digest`、`previous_digest`、`chain_digest`、安全的 `record_type/section/metric` 和时间戳，不保存行内容、候选 ID、trace ID、审计 ID 或行摘要明文解释。API 仅返回 `indexedRowCount`、`chainIntegrityStored=true` 和导出红线布尔值。

`test_design_report_archive_approval` 保存 `ARCHIVE` 和 `EXTERNAL_SHARE` 两类审批工单，包含固定原因编码、bounded 申请摘要、摘要 digest、工单 key/title/url/status、申请/审批人、状态和时间。归档审批通过后归档状态进入 `ARCHIVED`；外发审批受 `WP5_REPORT_ARCHIVE_EXTERNAL_SHARING_ALLOWED` 配置门禁，默认关闭时接口返回 `INVALID_STATE`。

`test_design_report_archive_note` 保存审批工单备注时间线，`note_type` 仅允许 `REQUEST`、`COMMENT`、`WORK_ORDER`、`REVIEW`，用于人工流转和审计。任务报告、任务诊断和归档 metadata 响应只输出备注计数和最近预览，不输出正文到 CSV。

约束与索引：

- manifest、归档、审批和备注均按任务/项目 scope 查询，审批相关接口通过归档或审批记录回溯项目权限。
- 行级完整性表对 `(archive_id, row_number)` 唯一，支持按归档统计索引完整度。
- DB validation 必须持续检查归档表不存在外部路径、候选 ID、trace ID、审计 ID、模型 payload 或行内容列，审批/备注表的自由文本均有长度约束。
...
服务端必须对 JSON 做结构校验和业务校验，不能把模型原始字符串直接入库为候选。`apiRefs`
属于非可信模型输出，仅当引用值是 UUID、能通过 WP3 应用服务解析为 API、且 API 所属项目等于当前
WP5 任务项目时，才允许写入候选 `apiId`；非 UUID、缺失 API 或跨项目 API 引用只忽略候选关联，
不得阻断整批候选生成或污染后续 WP3 发布请求。

### 5.1 上下文摘要契约

当前实现的 `context_summary_json` 只保存脱敏摘要，不保存完整 Prompt 或原始文档正文。任务创建时通过 WP3 应用服务读取需求、追踪链接、关联 API、页面、业务流和历史用例摘要；请求可额外传入 `contextApiIds/contextPageIds/contextFlowIds`，用于显式纳入未建立需求追踪关系但本次生成需要参考的上下文资产；请求也可传入 `environmentKey`，用于解析项目/环境级 effective context policy。上下文裁剪上限默认由 `veri-agent.test-design.context-*` 配置驱动；创建任务时按平台默认 -> 已审批项目覆盖 -> 已审批环境覆盖解析 effective limits，并写入 `contextSummary.limits`、`requestDigest`、模型请求 `contextPacking`、前端任务诊断和后端任务报告的聚合行，旧任务重试始终回放创建时的 `contextSummary` 快照，不受后续策略变化污染；`generationOrchestrationPolicy` 固定输出 `wp5-generation-orchestration-policy-v1`、同步/异步编排模式、`QUEUED -> RUNNING` 条件认领、创建幂等回放、重复事件安全、恢复扫描、运行中超时回收、显式重试、人工任务重试、人工排队事件重发、队列 lag 指标、超时聚合告警、恢复批次上限、队列 lag 阈值和运行超时阈值；health 和恢复扫描结果额外输出排队/运行/最旧排队年龄/超时运行聚合计数与告警布尔值，任务响应额外输出当前任务排队/运行/超时失败信号，`contextSummary.generationOrchestrationPolicy` 与模型 `contextPacking.generationOrchestrationPolicy` 只携带静态能力边界和阈值，不写入动态任务队列明细；该策略同步到 health、任务响应、`contextSummary.generationOrchestrationPolicy`、模型 `contextPacking.generationOrchestrationPolicy`、前端任务诊断和任务报告，并明确人工排队事件重发入口已就绪，多 worker 并发重复事件认领证据已纳入自动化测试；`contextAssemblyPolicy` v2 固定输出 `SNAPSHOT_DIGEST_ONLY` 装配模式、`SHA256_CONTEXT_SUMMARY` digest 策略、inputDigest 要求、摘要持久化、WP3 应用服务边界、原文/模型载荷持久化和上下文明细导出红线，并同步到 health、任务响应、`contextSummary.assemblyPolicy`、模型 `contextPacking.assemblyPolicy`、前端任务诊断和任务报告；`contextPolicyGovernance` 在 health 无项目场景保持平台默认治理状态，在任务 effective snapshot 中声明项目/环境覆盖能力、工单审批要求和任务创建时固化；`contextPolicyOperations` v2 在 health 无项目场景保持平台默认-only/工作流未就绪口径，在项目和任务场景输出 `PROJECT_ENVIRONMENT_OVERRIDE`、`PLATFORM_DEFAULT_PROJECT_ENVIRONMENT`、`FALLBACK_TO_PLATFORM_DEFAULT`、`WORK_ORDER_APPROVAL_READY`、覆盖存储就绪状态、工单审批流就绪状态和任务创建时快照固化状态，并同步到 health、任务响应、`contextSummary.policyOperations`、模型 `contextPacking.policyOperations`、前端任务诊断和任务报告；`scopePolicy` 固定输出 `PROJECT_RESOURCE_SCOPE`、列表无项目筛选时的平台级 fallback、任务/候选/批量/发布/异步生成/HTTP smoke/质量评测项目隔离标记，以及候选 ID、角色规则和服务令牌原值不导出标记，并同步到 health、任务响应、`contextSummary.scopePolicy`、模型 `contextPacking.scopePolicy`、前端任务诊断和任务报告；`evaluationCorpusPolicy` 固定输出 `GOLDEN_SET_BASELINE`、`MANUAL_OPT_IN_AI_EVAL`、`DEPLOY_CONFIG` 阈值来源、项目作用域、golden set 基线、AI 评测脚本、质量门禁接入、准出分布与 Prompt 版本跟踪，以及样本维护、长期校准和运营后台就绪状态，并同步到 health、任务响应、`contextSummary.evaluationCorpusPolicy`、模型 `contextPacking.evaluationCorpusPolicy`、前端任务诊断和任务报告；`releaseReadinessPolicy` 默认输出 `ADVISORY_QUALITY_GATE`、`DEPLOY_CONFIG` 阈值来源、质量阈值已评估、advisory-only、发布阻断关闭、人工准出要求、审批流就绪、自动发布关闭、候选确认要求、质量门禁例外支持和导出红线；开启 `veri-agent.test-design.release-readiness-publish-blocking-enabled=true` 后输出 `BLOCKING_QUALITY_GATE` 和发布阻断开启，正式发布在写入 WP3 前按任务聚合 readiness=`BLOCKED` 失败关闭；若存在与当前 readiness digest 匹配的已审批发布准出例外，则允许本次正式发布继续，dryRun 不受阻断影响；该策略同步到 health、任务响应、`contextSummary.releaseReadinessPolicy`、模型 `contextPacking.releaseReadinessPolicy`、前端任务诊断和任务报告；`auditChainPolicy` 固定输出 `wp5-audit-chain-policy-v1`、`WP5_DOMAIN_AGGREGATE_WITH_WP1_AUDIT`、`TASK_REVIEW_PUBLISH_MODEL_REFERENCES`、WP1 审计事件写入、WP2 调用引用跟踪、WP3 发布引用跟踪、WP5 本域事件跟踪、项目作用域、trace 信号、跨 WP 审计看板就绪、audit outbox 重放看板就绪和 aggregate-only 标记，并同步到 health、任务响应、`contextSummary.auditChainPolicy`、模型 `contextPacking.auditChainPolicy`、前端任务诊断和任务报告；`modelObservationPolicy` 固定输出 `wp5-model-observation-policy-v1`、`ROUTING_COST_LATENCY_AGGREGATE`、WP2 调用引用跟踪、trace/job/routing/token/latency/cost/fallback 跟踪能力、Prompt 载荷不存储、载荷预览不导出、trace/job/invocation ID 原值不导出、provider 错误正文不导出、actor service 不导出和 aggregate-only 标记，并同步到 health、任务响应、`contextSummary.modelObservationPolicy`、模型 `contextPacking.modelObservationPolicy`、前端任务诊断和任务报告；`archivePolicy` 固定输出 `wp5-archive-policy-v1`、有界保留天数、`platformManaged` 存储策略、审批要求、归档审批流就绪、外发审批流就绪、归档工单流转就绪、真实归档存储就绪、归档正文已存储、行级完整性索引就绪、外发开关、保留策略跟踪、归档内容/内部存储 key/归档路径/归档备注/审批说明/工单 URL 不导出和 aggregate-only 标记，并同步到 health、任务响应、`contextSummary.archivePolicy`、模型 `contextPacking.archivePolicy`、前端任务诊断和任务报告；`reportManifestPolicy` 固定输出 `wp5-report-manifest-policy-v1`、`wp5-task-report-v1`、`aggregate-only-v1`、`AGGREGATE_RECONCILIATION` 模式、行数/完成状态跟踪、归档核验就绪、行级完整性存储/index 就绪、明细行/行级完整性值/行内容摘要/候选 ID/trace ID/审计 ID 不导出和 aggregate-only 标记，并同步到 health、任务响应、`contextSummary.reportManifestPolicy`、模型 `contextPacking.reportManifestPolicy`、前端任务诊断和任务报告。WP5 不直连 WP3 表，也不在 `auditChainPolicy` 中查询或导出全局 `audit_log` 明细；本阶段已创建 context policy override 元数据表、审批工单与备注时间线表、bounded 策略正文和 diff 管理、发布准出例外审批元数据表、portal-web 策略运营与发布准出审批面板、aggregate-only 报告 manifest 持久化记录、托管归档正文存储、归档审批/外发审批工单、归档备注时间线和报告行级完整性索引；模型观测明细仍只提供聚合钻取，不导出原始 payload 或标识值。
...
```json
{
  ...
  "policyGovernance": {
    ...
    "governanceStatus": "OVERRIDE_APPROVED",
  ...
  "policyOperations": {
    ...
    "approvalStatus": "WORK_ORDER_APPROVAL_READY",
...
| `GET` | `/operations/cross-wp-dashboard` | `testDesign:read` | 返回跨 WP 统一运营看板，按项目和 Prompt 聚合 scope 覆盖、WP1 审计、WP2 调用、WP3 发布引用、audit outbox、队列告警、补偿、批量运营审计、审计报表模板、模型观测钻取和跨 WP 脱敏明细审计；只输出 aggregate-only 指标。 |
| `GET` | `/operations/queue-alert-subscriptions` | `testDesign:read` | 查询项目/prompt scope 下的队列告警订阅，只返回 bounded targetRef 和审计元数据。 |
| `POST` | `/operations/queue-alert-subscriptions` | `testDesign:policy_manage` | 创建或更新一条队列告警订阅；请求只能携带 bounded targetRef、渠道、阈值和启停状态。 |
| `POST` | `/operations/queued-events/replay` | `testDesign:policy_manage` | 按项目/prompt 有界重放 queued generation/publish 事件，只返回重放计数。 |
| `GET` | `/operations/compensation-runbook` | `testDesign:read` | 返回发布补偿运行手册，说明候选选择边界、手工运行条件和聚合就绪状态。 |
| `POST` | `/operations/publish-compensation/run` | `testDesign:policy_manage` | 按项目/prompt 触发手工发布补偿运行，只返回扫描/成功/失败/跳过计数。 |
| `GET` | `/operations/audit-report` | `testDesign:read` | 返回批量运营审计报表，聚合队列告警订阅变更、queued replay、发布补偿运行和 audit outbox requeue。 |
| `GET` | `/operations/audit-report-template` | `testDesign:read` | 返回固定审计报表模板和字段集，声明运营汇总、告警订阅、模型观测聚合钻取、跨 WP 脱敏明细和导出红线字段。 |
| `GET` | `/operations/model-observation-drilldown` | `testDesign:read` | 返回模型观测聚合钻取，按 status/provider/model/routing group/prompt version/fallback 输出调用、token、成本、延迟和 trace/job 存在性计数。 |
| `GET` | `/operations/cross-wp-detail-audit-report` | `testDesign:read` | 返回跨 WP 脱敏明细审计报表，按 WP 来源、分类和状态输出聚合行，不返回原始审计行或标识原值。 |
| `POST` | `/operations/audit-outbox/requeue` | `testDesign:policy_manage` | 按项目 scope 对 `FAILED/DEAD` audit outbox 执行 bounded requeue；请求必须带 `projectId`，响应只返回重排队数量和导出红线。 |
...
`/operations/cross-wp-dashboard` 响应固定包含 `scopePolicy.crossWpScopeDashboardReady=true`、`auditChainPolicy.crossWpAuditDashboardReady=true`、`auditChainPolicy.auditOutboxReplayDashboardReady=true`、`aggregateOnly=true` 和 `detailIdentifiersExported=false`。`auditDashboard` 与 `auditOutbox` 只允许返回计数、ready 布尔值和导出红线；不得返回审计事件行、outbox payload、lastError、traceId 原值、模型调用 ID、候选 ID、发布 sourceRef、WP3 资产 ID、角色规则明细或服务令牌原值。`/operations/audit-outbox/requeue` 必须按项目 scope、状态和 limit 受限更新 `FAILED/DEAD` 为 `PENDING`，并写操作审计；响应不回显 reason 原文。

`/operations/queue-alert-subscriptions` 和 `/operations/audit-report` 一律只返回聚合计数、bounded targetRef、启停状态和审计元数据；不得返回 webhook URL、token、payload、任务 ID、候选 ID、sourceRef、outbox payload、traceId 原值或模型调用 ID。`/operations/audit-report-template` 只返回固定字段定义和导出模式，不返回任何运行时标识原值。`/operations/model-observation-drilldown` 只返回聚合桶，不返回 traceId/jobId/invocationId 原值、provider 错误正文、actor service 或载荷预览。`/operations/cross-wp-detail-audit-report` 只返回脱敏明细桶，不返回 WP1 审计行、WP2 调用行、WP3 资产标识、任务 ID、候选 ID、sourceRef、payload 或 actor 标识。`/operations/queued-events/replay` 和 `/operations/publish-compensation/run` 只返回重放/补偿扫描计数和成功计数，不回显 event payload、候选 ID、资产 ID、sourceRef 或错误正文。`/operations/compensation-runbook` 只返回运行边界、阈值、候选范围和就绪状态，不回显明细标识或人工备注。

`/tasks/{id}/report/audit-chain` 响应固定包含 `readOnlyAggregateDashboardReady=true`、`crossWpAuditDashboardReady=true`、`auditOutboxReplayDashboardReady=true` 和 `aggregateOnly=true`。`metrics` 仅允许返回聚合计数和语义 tone，`readiness` 仅允许返回就绪布尔值和固定说明；仓储 SQL 必须把 `audit_outbox` 计数限定在当前任务、候选或发布用例相关资源上，不得输出全局 outbox 运营计数。
...
| `PUT` | `/context-policies/overrides/{id}` | `testDesign:policy_manage` | 更新 PENDING 覆盖草稿、审批工单、bounded 策略正文和策略 diff。 |
...
| `GET` | `/context-policies/overrides/{id}/notes` | `testDesign:read` | 查询策略覆盖审批工单备注时间线。 |
| `POST` | `/context-policies/overrides/{id}/notes` | `testDesign:policy_manage` | 追加 COMMENT 或 WORK_ORDER 备注。 |
...
```json
{
  ...
  "workOrderKey": "WP5-CTX-1001",
  "workOrderTitle": "上下文裁剪策略调整",
  "workOrderUrl": "https://change.example.com/WP5-CTX-1001",
  "policyBody": "仅保存 bounded 策略正文，不保存上下文原文。",
  "policyDiffSummary": "提高需求摘要上限。",
  "requestNote": "项目复杂度提升，需要增加摘要窗口。"
}
```
...
```json
{
  ...
  "reviewNote": "审批通过，后续新任务生效。",
  "workOrderStatus": "APPROVED"
}
```

覆盖响应只在运营接口返回 bounded 策略正文、diff 摘要、工单元数据和备注预览；任务诊断、模型上下文和任务报告不得导出这些运营明细：
...
portal-web 的 WP5 工作台提供上下文策略运营面板：
...
- `testDesign:policy_manage` 用户可选择 PROJECT/ENVIRONMENT 范围、固定原因编码、bounded 数字上限、审批工单、策略正文、策略 diff 和申请备注，提交或更新 PENDING 覆盖。
- `testDesign:policy_manage` 用户可对 PENDING 覆盖执行 approve/reject，维护审批原因、审批备注和工单状态，并追加 COMMENT/WORK_ORDER 备注时间线。
- 面板、API helper 和测试只处理 bounded 数字、状态、时间、申请/审批人、工单元数据、bounded 策略正文/diff 和备注；不得展示或构造上下文正文、原因编码自由文本、候选明细或可反查上下文明细。

### 6.7 发布准出审批 API

| 方法 | 路径 | 权限 | 说明 |
|---|---|---|---|
| `GET` | `/tasks/{id}/release-readiness/approvals` | `testDesign:read` | 查询任务级发布准出例外审批记录。 |
| `POST` | `/tasks/{id}/release-readiness/approvals` | `testDesign:publish` | 在当前任务 readiness=`BLOCKED` 时创建 PENDING 质量门禁例外审批。 |
| `PUT` | `/release-readiness/approvals/{id}` | `testDesign:publish` | 更新 PENDING 发布准出例外草稿。 |
| `POST` | `/release-readiness/approvals/{id}/approve` | `testDesign:publish` | 审批 PENDING 例外，审批时重新捕获当前 readiness digest。 |
| `POST` | `/release-readiness/approvals/{id}/reject` | `testDesign:publish` | 驳回 PENDING 例外并保留审计记录。 |
| `GET` | `/release-readiness/approvals/{id}/notes` | `testDesign:read` | 查询发布准出审批备注时间线。 |
| `POST` | `/release-readiness/approvals/{id}/notes` | `testDesign:publish` | 追加 COMMENT 或 WORK_ORDER 备注。 |

创建例外请求只允许固定原因编码、bounded 文本和工单元数据：

```json
{
  "exceptionReasonCode": "SMOKE_VALIDATION",
  "exceptionSummary": "已完成阻断项复核，本次仅放行当前聚合质量状态。",
  "riskMitigation": "例外绑定当前 readiness digest，候选变化后需重新审批。",
  "workOrderKey": "WP5-RR-1001",
  "workOrderTitle": "发布准出质量门禁例外",
  "workOrderUrl": "https://change.example.com/WP5-RR-1001",
  "requestNote": "申请一次性放行。"
}
```
...
{
  "approvalReasonCode": "SMOKE_VALIDATION",
  "reviewNote": "审批通过，仅对当前 readiness digest 生效。",
  "workOrderStatus": "APPROVED"
}
```

审批通过只放行与当前任务聚合 readiness digest 完全匹配的 `APPROVED` 记录；候选编辑、发布失败或质量阈值变化导致 digest 改变后，正式发布仍按 readiness=`BLOCKED` 失败关闭并要求重新审批。发布准出运营接口可回显原因编码、工单、例外摘要、风险缓释和备注预览；任务诊断、模型上下文、任务报告和 CSV 导出仍只输出 `releaseReadinessPolicy` 聚合布尔与 readiness 聚合计数，不导出候选级准出证据、审批备注或阈值规则明细。

### 6.8 报告归档与外发审批 API

| 方法 | 路径 | 权限 | 说明 |
|---|---|---|---|
| `GET` | `/tasks/{id}/report/archives` | `testDesign:read` | 查询任务报告归档元数据，只返回 digest、行数、状态和导出红线。 |
| `GET` | `/report-archives/{id}/integrity` | `testDesign:read` | 查询归档行级完整性聚合摘要，不返回行 digest 或行内容。 |
| `GET` | `/report-archives/{id}/approvals` | `testDesign:read` | 查询归档记录下的 ARCHIVE/EXTERNAL_SHARE 审批工单。 |
| `POST` | `/report-archives/{id}/archive-approvals` | `testDesign:export` | 手工补建归档审批工单；同一归档存在 PENDING 工单时返回 `INVALID_STATE`。 |
| `POST` | `/report-archives/{id}/external-approvals` | `testDesign:export` | 为已归档报告申请外发审批；受 `WP5_REPORT_ARCHIVE_EXTERNAL_SHARING_ALLOWED` 配置门禁。 |
| `POST` | `/report-archive-approvals/{id}/approve` | `testDesign:export` | 审批 PENDING 归档或外发工单。 |
| `POST` | `/report-archive-approvals/{id}/reject` | `testDesign:export` | 驳回 PENDING 归档或外发工单。 |
| `GET` | `/report-archive-approvals/{id}/notes` | `testDesign:read` | 查询归档审批工单备注时间线。 |
| `POST` | `/report-archive-approvals/{id}/notes` | `testDesign:export` | 追加 COMMENT 或 WORK_ORDER 备注。 |

创建申请只允许固定原因编码、bounded 申请摘要和工单元数据：

```json
{
  "reasonCode": "RETENTION_CONTROL",
  "requestSummary": "任务报告进入正式归档。",
  "workOrderKey": "WP5-ARCH-1001",
  "workOrderTitle": "WP5 报告归档审批",
  "workOrderUrl": "https://change.example.com/WP5-ARCH-1001",
  "requestNote": "申请归档。"
}
```

归档 API 响应不得返回 `contentBytes`、`storageKey`、`rowDigest`、`previousDigest`、`chainDigest`、候选 ID、trace ID 或审计 ID；DB validation 与 API 测试必须同时约束该红线。
