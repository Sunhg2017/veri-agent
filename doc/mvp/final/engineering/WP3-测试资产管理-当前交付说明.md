...
7. WP4 发布：`IMPORT + sourceRef` 需求幂等写入，重复导入在 DRAFT 状态下更新，非 DRAFT 差异阻断。

## 2. 非范围

本轮不实现完整版本历史、资产 diff 页面、软删除恢复、CSV/OpenAPI 导入导出、API 资产 OpenAPI 导入执行能力、追踪矩阵影响分析和企业原型连接器真实拉取。这些能力保留在后续 P1/P2 任务中。
...
`portal-web` 已增加资产库入口、需求资产工作台和 API 资产工作台：
...
后续优先补齐版本历史、软删除恢复、导入导出、OpenAPI 导入执行能力、完整追踪矩阵，以及页面/业务流/用例前端工作台。
