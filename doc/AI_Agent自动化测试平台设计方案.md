...
> 历史归档说明：本文记录早期总体蓝图，不代表当前实现口径。当前实现以 `doc/mvp/final/engineering/当前实现基线.md`、代码、迁移脚本和自动化测试为准：后端为单个 `platform-api` Java 服务，WP1/WP2/WP3 是同服务内领域模块，不建设多租户，API 使用 camelCase，分页使用 `index/size`。
