...
public class ApiAutomationPermissionScopeResolver {
    ...
    public ResourceScope generationTaskList(ApiAutomationGenerationTaskPageRequest request) {
        if (request != null && StringUtils.hasText(request.getProjectId())) {
            return project(request.getProjectId());
        }
        if (request != null && request.getSpecId() != null) {
            return spec(request.getSpecId());
        }
        return ResourceScope.platform();
    }
    ...
    public ResourceScope runRequest(CreateApiAutomationRunCommand command) {
        if (command != null && command.bundleId() != null) {
            return scriptBundle(command.bundleId());
        }
        return ResourceScope.platform();
    }

    public ResourceScope run(UUID id) {
        return ResourceScope.project(scopeService.runProjectScopeId(id));
    }
}
