...
class PlatformIntegrationControllerTest {
    ...
    void exposesContextValidationForServiceCallers() throws Exception {
        mockMvc.perform(get("/api/v1/contexts/projects/project-001")
                ...
                .andExpect(jsonPath("$.data.resource_type").value("PROJECT"))
                .andExpect(jsonPath("$.data.resource_id").value("project-001"))
                .andExpect(jsonPath("$.data.sensitivity_level").value("INTERNAL"))
                .andExpect(jsonPath("$.data.allow_public_model").value(false))
