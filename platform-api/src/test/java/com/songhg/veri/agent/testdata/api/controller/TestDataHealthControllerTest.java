...
class TestDataHealthControllerTest {
    ...
    void exposesControlPlaneHealthWithoutSensitiveValues() throws Exception {
        mockMvc.perform(get("/api/v1/test-data/health"))
                ...
                .andExpect(jsonPath("$.data.policy.workerManagedByXxlJob").value(false))
                .andExpect(jsonPath("$.data.policy.workerRuntimeReady").value(false))
                .andExpect(jsonPath("$.data.policy.cleanupWorkerReady").value(false))
                .andExpect(jsonPath("$.data.policy.cleanupExecutionReady").value(false))
                .andExpect(jsonPath("$.data.policy.taskExecutionWorkerReady").value(false))
                .andExpect(jsonPath("$.data.policy.leaseRecoveryWorkerReady").value(false))
                .andExpect(jsonPath("$.data.policy.accountHealthCheckWorkerReady").value(false))
                ...
                .andExpect(jsonPath("$.data.policy.accountProvisioningRealAdapterRequired").value(true))
                .andExpect(jsonPath("$.data.policy.accountProvisioningLocalSecretRefMode").value(false))
