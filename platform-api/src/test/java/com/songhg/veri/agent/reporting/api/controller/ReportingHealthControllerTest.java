...
class ReportingHealthControllerTest {
    ...
    void exposesM1ControlPlaneHealthWithoutSensitiveValues() throws Exception {
        mockMvc.perform(get("/api/v1/reports/health"))
                ...
                .andExpect(jsonPath("$.data.asyncGenerationEnabled").value(false))
                .andExpect(jsonPath("$.data.generationWorkerEnabled").value(true))
                .andExpect(jsonPath("$.data.generationWorkerId").value("wp10-report-worker"))
                .andExpect(jsonPath("$.data.generationWorkerBatchSize").value(4))
                .andExpect(jsonPath("$.data.generationRunningTimeoutSeconds").value(1800))
                .andExpect(jsonPath("$.data.generationRecoveryBatchSize").value(50))
                ...
                .andExpect(jsonPath("$.data.policy.asyncGenerationRequestModeReady").value(true))
                .andExpect(jsonPath("$.data.policy.asyncGenerationWorkerReady").value(true))
                .andExpect(jsonPath("$.data.policy.generationWorkerUsesStatusClaim").value(true))
                .andExpect(jsonPath("$.data.policy.generationWorkerRecoversStaleGenerating").value(true))
                .andExpect(jsonPath("$.data.policy.asyncGenerationDefaultDisabled").value(true))
                ...
                .andExpect(jsonPath("$.data.policy.wp3EvidenceManifestReady").value(true))
                .andExpect(jsonPath("$.data.policy.wp5EvidenceManifestReady").value(true))
                .andExpect(jsonPath("$.data.policy.evidenceAggregationReady").value(true))
