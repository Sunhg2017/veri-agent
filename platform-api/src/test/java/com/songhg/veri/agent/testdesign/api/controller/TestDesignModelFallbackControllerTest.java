...
class TestDesignModelFallbackControllerTest {
    ...
    @Test
    void fallsBackToRuleTemplateWhenWp2PromptIsUnavailable() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/test-design/tasks")
                ...
                .andExpect(jsonPath("$.data.task.modelInvocationId").doesNotExist())
                .andExpect(jsonPath("$.data.task.errorMessage", containsString("已降级规则模板")))
                .andExpect(jsonPath("$.data.candidates", hasSize(1)))
                .andExpect(jsonPath("$.data.candidates[0].title", startsWith("验证模型降级需求")))
                .andExpect(jsonPath("$.data.candidates[0].modelInvocationId").doesNotExist());
    }
    ...
    private String createRequirement(String userToken) throws Exception {
        MvcResult result = mockMvc.perform(post("/api/v1/asset/requirements")
                        ...
                        .content("""
                                {
                                  "title": "模型降级需求",
                                  "description": "WP5 模型失败后仍可生成模板候选",
                                  "priority": "HIGH",
                                  "projectId": "project-wp5",
                                  "acceptanceCriteria": "模板候选可人工评审"
                                }
                                """))
                .andExpect(status().isCreated())
                .andReturn();
    ...
    private String userAccessToken(List<String> roles) {
        return tokenService.issue(new AuthUserRecord(
                UUID.randomUUID(),
                "wp5_model_fallback_user",
                "WP5 Model Fallback User",
                "wp5-model-fallback@example.com",
                "$2a$10$test",
                false,
                1,
                roles
