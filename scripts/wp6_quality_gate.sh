...
runner_smoke_requested() {
  case "${WP6_RUNNER_SMOKE:-0}" in
    1|true|TRUE|managed|pytest|sandbox|docker-sandbox|pytest-docker-sandbox|auto|external)
      return 0
      ;;
    *)
      return 1
...
validate_release_gate() {
  ...
  if ! runner_smoke_requested; then
    echo "WP6 release gate requires runner smoke. Set WP6_RUNNER_SMOKE=managed, pytest or sandbox for local smoke, or WP6_RUNNER_SMOKE=external with a reviewed runner base URL." >&2
    exit 2
  fi
...
run_runner_smoke() {
  case "${WP6_RUNNER_SMOKE:-0}" in
    1|true|TRUE|managed|pytest|sandbox|docker-sandbox|pytest-docker-sandbox|auto|external)
      ...
      ;;
    0|false|FALSE|"")
      echo "== wp6 runner smoke skipped; set WP6_RUNNER_SMOKE=managed, pytest, sandbox or external for explicit runner smoke =="
      ;;
    *)
      echo "Unsupported WP6_RUNNER_SMOKE=${WP6_RUNNER_SMOKE}; use managed, pytest, sandbox, auto, external, 1, or 0." >&2
      exit 2
      ;;
  esac
}
