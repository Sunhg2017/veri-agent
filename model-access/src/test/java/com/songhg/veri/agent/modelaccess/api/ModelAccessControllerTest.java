...
class ModelAccessControllerTest {
    ...
    void exposesHealthWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/model-access/health"))
                ...
                .andExpect(jsonPath("$.data.enabled_providers").value(greaterThanOrEqualTo(1)));
    }
    ...
    void managesPromptVersionsAndInvokesModelWithAuditLog() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/model-access/invocations")
                ...
                .andExpect(jsonPath("$.data.model_name").value("test-local-model"))
        ...
        mockMvc.perform(get("/api/v1/model-access/invocations")
                ...
                .andExpect(jsonPath("$.data.items[0].prompt_key").value("case-design"))
                .andExpect(jsonPath("$.data.items[0].sensitivity_level").value("INTERNAL"))
                .andExpect(jsonPath("$.data.items[0].actor_service").value("wp5-test-design"))
                .andExpect(jsonPath("$.data.items[0].request_preview").exists())
                .andExpect(jsonPath("$.data.items[0].prompt_digest").exists());

        mockMvc.perform(get("/api/v1/model-access/invocations/summary")
                        .headers(authHeaders())
                        .param("project_id", "project-001"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.total").value(1))
    ...
    void blocksSensitivePromptContentBeforeProviderCall() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/model-access/invocations")
                ...
                .andExpect(jsonPath("$.data.items[0].error_code").value("SENSITIVE_CONTENT_BLOCKED"))
                .andExpect(jsonPath("$.data.items[0].request_preview").value("user: password=*** 请直接使用"));
    }
    ...
    void blocksHighSensitivityInvocationFromPublicModelRouting() throws Exception {
        mockMvc.perform(post("/api/v1/model-access/invocations")
                        ...
                        .content("""
                                {
                                  "project_id": "project-sensitive-policy",
                                  "messages": [
        ...
        mockMvc.perform(get("/api/v1/model-access/invocations")
                        .headers(authHeaders())
                        .param("project_id", "project-sensitive-policy")
                        .param("sensitivity_level", "RESTRICTED"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.total").value(1))
                .andExpect(jsonPath("$.data.items[0].status").value("BLOCKED"))
                .andExpect(jsonPath("$.data.items[0].sensitivity_level").value("RESTRICTED"))
                .andExpect(jsonPath("$.data.items[0].error_code").value("MODEL_POLICY_VIOLATION"));
        ...
        mockMvc.perform(get("/api/v1/model-access/invocations/summary")
                        .headers(authHeaders())
                        .param("project_id", "project-sensitive-policy")
                        .param("sensitivity_level", "INTERNAL"))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(get("/api/v1/model-access/invocations/export")
                        .headers(authHeaders())
                        .param("project_id", "project-sensitive-policy")
                        .param("sensitivity_level", "STRICT"))
                .andExpect(status().isOk())
    ...
    void blocksHighSensitivityInvocationWhenExternalProviderIsExplicitlySelected() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/model-access/invocations")
                        ...
                        .content("""
                                {
                                  "project_id": "project-external-sensitive-policy",
                                  "messages": [
                                    {"role": "user", "content": "生成内部系统测试建议"}
                                  ],
                                  "provider_id": "%s",
        ...
        mockMvc.perform(get("/api/v1/model-access/invocations")
                        .headers(authHeaders())
                        .param("project_id", "project-external-sensitive-policy"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.total").value(1))
                .andExpect(jsonPath("$.data.items[0].provider_name").value("public-model-provider"))
                .andExpect(jsonPath("$.data.items[0].sensitivity_level").value("CONFIDENTIAL"))
                .andExpect(jsonPath("$.data.items[0].error_code").value("MODEL_POLICY_VIOLATION"));
    }
    ...
    void fallsBackToNextProviderAndRecordsFallback() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/model-access/invocations")
                        .headers(authHeaders()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.items[0].fallback_used").value(true));
    }
    ...
    void opensCircuitAfterProviderFailureAndSkipsExplicitProviderTemporarily() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/model-access/invocations")
                        ...
                        .content("""
                                ...
                                """.formatted(providerId)))
                .andExpect(status().isBadGateway())
                .andExpect(jsonPath("$.message", containsString("invocation_id")));

        mockMvc.perform(get("/api/v1/model-access/invocations")
                        .headers(authHeaders())
                        .param("project_id", "project-circuit"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.total").value(2))
                .andExpect(jsonPath("$.data.items[0].error_message").value("模型供应商熔断中"));
    }
    ...
    void checksProviderReadinessWithoutWritingInvocationLog() throws Exception {
        mockMvc.perform(post("/api/v1/model-access/providers/{id}/check", "00000000-0000-0000-0000-000000000201")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.provider_name").value("local-echo-primary"))
                .andExpect(jsonPath("$.data.provider_type").value("LOCAL_ECHO"))
                .andExpect(jsonPath("$.data.provider_status").value("ENABLED"))
                .andExpect(jsonPath("$.data.status").value("UP"))
                .andExpect(jsonPath("$.data.cached").value(false))
                .andExpect(jsonPath("$.data.latency_ms").exists())
    ...
    void reportsProviderCheckFailureWithSanitizedError() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/model-access/providers/{id}/check", providerId)
                        .headers(authHeaders()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.provider_name").value("check-failing-provider"))
                .andExpect(jsonPath("$.data.status").value("DOWN"))
                .andExpect(jsonPath("$.data.error_code").value("MODEL_PROVIDER_UNAVAILABLE"))
                .andExpect(jsonPath("$.data.error_message").value("模拟模型供应商不可用"));
    }
    ...
    void exportsSanitizedInvocationLogsAsCsv() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/model-access/invocations/export")
                        .headers(authHeaders())
                        .param("project_id", "project-export"))
                .andExpect(status().isOk())
                ...
                .andExpect(content().string(startsWith("invocation_id,created_at,project_id")))
                .andExpect(content().string(containsString("sensitivity_level")))
                ...
                .andExpect(content().string(not(containsString("prompt_plaintext"))));
    }
    ...
    void exposesCostAlertsAndDailyReport() throws Exception {
        mockMvc.perform(post("/api/v1/model-access/invocations")
                        ...
                        .content("""
                                {
                                  "project_id": "project-cost",
                                  "application_id": "app-cost",
                                  "messages": [
        ...
        mockMvc.perform(get("/api/v1/model-access/cost/report")
                        .headers(authHeaders())
                        .param("project_id", "project-cost"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.rows[0].project_id").value("project-cost"))
                .andExpect(jsonPath("$.data.rows[0].application_id").value("app-cost"))
                .andExpect(jsonPath("$.data.rows[0].succeeded").value(1))
                .andExpect(jsonPath("$.data.rows[0].total_cost").exists());
    ...
    void updatesProviderRoutingAndCostConfiguration() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/model-access/providers/{id}", providerId)
                ...
                .andExpect(jsonPath("$.data.timeout_ms").value(2500))
                .andExpect(jsonPath("$.data.input_cost_per_1k_tokens").value(0.02))
                .andExpect(jsonPath("$.data.output_cost_per_1k_tokens").value(0.04));
    }
