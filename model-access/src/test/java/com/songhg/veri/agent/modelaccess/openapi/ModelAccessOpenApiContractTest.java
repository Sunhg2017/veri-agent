...
class ModelAccessOpenApiContractTest {
    ...
    void exposesWp2ModelAccessContractWithBearerSecurity() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(jsonPath("$.info.title").value("Veri Agent WP2 Model Access API"))
    ...
    void generatedContractDoesNotExposeSecretPlaintextFields() throws Exception {
        ...
        MatcherAssert.assertThat(openApi, containsString("api_key_ref"));
        MatcherAssert.assertThat(openApi, not(containsString("api_key_value")));
        MatcherAssert.assertThat(openApi, not(containsString("secret_value")));
        MatcherAssert.assertThat(openApi, not(containsString("prompt_plaintext")));
    }
    ...
    void generatedContractKeepsInvocationAuditFiltersAndCsvExportVisible() throws Exception {
        ...
        String openApi = result.getResponse().getContentAsString();
        MatcherAssert.assertThat(openApi, containsString("sensitivity_level"));
        MatcherAssert.assertThat(openApi, containsString("actor_service"));
