...
import com.songhg.veri.agent.modelaccess.common.TraceContext;
...
public class MockPlatformContextClient implements PlatformContextClient {
    ...
    public void writeInvocationAudit(InvocationRecord record) {
        ...
        try {
            ...
            log.warn("Failed to write WP1 audit event for invocation_id={}, trace_id={}",
    ...
    private PlatformInvocationPolicy readPolicy(JsonNode envelope) {
        ...
        String sensitivityLevel = data == null || data.path("sensitivity_level").isMissingNode()
                ? "INTERNAL"
                : data.path("sensitivity_level").asText("INTERNAL");
        boolean allowPublicModel = data != null && data.path("allow_public_model").asBoolean(false);
