...
import com.songhg.veri.agent.modelaccess.api.CreatePromptRequest;
import com.songhg.veri.agent.modelaccess.api.CreateProviderRequest;
import com.songhg.veri.agent.modelaccess.api.CostAlertResponse;
import com.songhg.veri.agent.modelaccess.api.CostReportResponse;
import com.songhg.veri.agent.modelaccess.api.InvokeModelRequest;
import com.songhg.veri.agent.modelaccess.api.InvokeModelResponse;
...
import com.songhg.veri.agent.modelaccess.api.UpdateProviderRequest;
...
public class ModelAccessService {
    ...
    public InvokeModelResponse invoke(InvokeModelRequest request, ServicePrincipal principal) {
        ...
        throw new BusinessException(
                ...
                "模型调用失败，invocation_id=" + failed.id()
        );
    }

    public List<InvocationRecord> invocations() {
        return repository.invocations(new InvocationQuery(null, null, null, null, null, null, null, null, 0, 200));
    }
    ...
    public PageResponse<InvocationRecord> invocations(InvocationQuery query) {
        ...
        return PageResponse.of(items, normalized.page(), normalized.size(), total);
    }
    ...
    public List<CostAlertResponse> costAlerts(String projectId) {
        ...
        if (properties.hasDailyPlatformCostLimit()) {
            alerts.add(costAlert(
                    ...
                    new InvocationQuery(null, null, null, null, null, null, window.startTime(), window.endTime(), 0, 1),
                    window
            ));
        }
        if (properties.hasDailyProjectCostLimit()) {
            ...
            if (normalizedProjectId == null) {
                repository.invocations(new InvocationQuery(null, null, null, null, null, null, window.startTime(), window.endTime(), 0, 1000))
                        .stream()
    ...
    public CostReportResponse costReport(LocalDate startDate, LocalDate endDate, String projectId) {
        ...
        InvocationQuery query = new InvocationQuery(
                ...
                0,
                Math.max(1, properties.maxExportRows() <= 0 ? 10000 : Math.min(50000, properties.maxExportRows()))
        );
    ...
    public String exportInvocationsCsv(InvocationQuery query) {
        ...
        InvocationQuery exportQuery = new InvocationQuery(
                ...
                normalized.endTime(),
                0,
                Math.min(50000, exportRows)
        );
        StringBuilder csv = new StringBuilder();
        csv.append("invocation_id,created_at,project_id,application_id,environment_id,sensitivity_level,status,")
                .append("provider_id,provider_name,model_name,prompt_key,prompt_version,fallback_used,")
                .append("prompt_digest,input_tokens,output_tokens,total_cost,latency_ms,actor_service,")
                .append("delegated_user_id,error_code,error_message,request_preview,response_preview\n");
    ...
    private String sensitivityLevel(String sensitivityLevel) {
        ...
        return switch (normalized) {
            ...
            default -> throw new BusinessException(ErrorCode.BAD_REQUEST, "sensitivity_level 仅支持 PUBLIC/INTERNAL/CONFIDENTIAL/RESTRICTED");
        };
    }
    ...
    private void validateProviderConfig(ModelProviderConfig provider) {
        ...
        if (provider.providerType() == ProviderType.OPENAI_COMPATIBLE) {
            if (!StringUtils.hasText(provider.baseUrl())) {
                throw new BusinessException(ErrorCode.BAD_REQUEST, "OpenAI-compatible 供应商必须配置 base_url");
            }
            ...
            if (!StringUtils.hasText(provider.apiKeyRef()) || !provider.apiKeyRef().startsWith("env:")) {
                throw new BusinessException(ErrorCode.BAD_REQUEST, "OpenAI-compatible 供应商 api_key_ref 必须使用 env:VARIABLE_NAME");
            }
        }
    }
    ...
    private void validateExternalProviderBaseUrl(String baseUrl) {
        try {
            ...
            if (!"http".equalsIgnoreCase(scheme) && !"https".equalsIgnoreCase(scheme)) {
                throw new BusinessException(ErrorCode.BAD_REQUEST, "OpenAI-compatible 供应商 base_url 仅支持 http/https");
            }
            if (!StringUtils.hasText(uri.getHost()) || StringUtils.hasText(uri.getUserInfo())) {
                throw new BusinessException(ErrorCode.BAD_REQUEST, "OpenAI-compatible 供应商 base_url 格式无效");
            }
    ...
    private InvocationQuery normalizeQuery(InvocationQuery query) {
        ...
        int size = Math.min(200, Math.max(1, query.size()));
    ...
    private CostAlertResponse projectCostAlert(String projectId, BudgetWindow window) {
        return costAlert(
                ...
                projectId,
                properties.dailyProjectCostLimit(),
                new InvocationQuery(projectId, null, null, null, null, null, window.startTime(), window.endTime(), 0, 1),
                window
        );
    }
    ...
    private BudgetReportWindow normalizeReportWindow(LocalDate startDate, LocalDate endDate) {
        ...
        if (safeStart.isAfter(safeEnd)) {
            throw new BusinessException(ErrorCode.BAD_REQUEST, "start_date 不能晚于 end_date");
        }
