...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleAuthentication(AuthenticationException exception) {
        return ResponseEntity
                ...
                .body(ApiResponse.error(ErrorCode.UNAUTHORIZED.name(), "认证失败或服务令牌已失效", TraceContext.getTraceId(), null));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleAccessDenied(AccessDeniedException exception) {
        return ResponseEntity
                ...
                .body(ApiResponse.error(ErrorCode.FORBIDDEN.name(), "权限不足", TraceContext.getTraceId(), null));
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiResponse<Void>> handleUnexpected(Exception exception) {
        log.error("Unexpected model-access exception, trace_id={}", TraceContext.getTraceId(), exception);
        return ResponseEntity
                .status(ErrorCode.INTERNAL_ERROR.httpStatus())
                .body(ApiResponse.error(ErrorCode.INTERNAL_ERROR.name(), "系统异常", TraceContext.getTraceId(), null));
    }
